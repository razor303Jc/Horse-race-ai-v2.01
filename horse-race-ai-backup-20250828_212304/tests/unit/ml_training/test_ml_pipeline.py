...
class TestMLModelTraining:
    ...
    @patch('docker.ml_training.real_ml_training_pipeline.joblib.dump')
    def test_random_forest_training(self, mock_joblib_dump):
        """Test Random Forest model training"""
        # Create training data
        training_data = pd.DataFrame({
            'age': [4, 5, 6, 4, 5, 6, 4, 5],
            'or_rating': [85, 90, 88, 86, 91, 87, 84, 89],
            'runners': [8, 8, 8, 10, 10, 10, 12, 12],
            'is_favorite': [1, 0, 0, 1, 0, 1, 0, 0],
            'sp': [3.0, 5.5, 7.0, 2.8, 6.0, 3.2, 8.0, 4.5],
            'won': [1, 0, 0, 1, 0, 1, 0, 0]  # Target variable
        })
        ...
        if ML_PIPELINE_AVAILABLE:
            ...
            # Mock ML libraries
            with patch('docker.ml_training.real_ml_training_pipeline.RandomForestClassifier') as mock_rf:
                with patch('docker.ml_training.real_ml_training_pipeline.train_test_split') as mock_split:
                    # Setup mocks
                    mock_model = Mock()
    ...
    def test_logistic_regression_training(self, mock_joblib_dump):
        ...
        if ML_PIPELINE_AVAILABLE:
            ...
            with patch('docker.ml_training.real_ml_training_pipeline.LogisticRegression') as mock_lr:
                with patch('docker.ml_training.real_ml_training_pipeline.train_test_split') as mock_split:
                    # Setup mocks
                    mock_model = Mock()
                    mock_model.predict.return_value = np.array([1, 0, 1])
                    mock_model.predict_proba.return_value = np.array([[0.3, 0.7], [0.8, 0.2], [0.2, 0.8]])
...
class TestMLPipelineIntegration:
    ...
    @patch('docker.ml_training.real_ml_training_pipeline.psycopg2.connect')
    ...
    @patch('docker.ml_training.real_ml_training_pipeline.pd.read_sql')
    @patch('docker.ml_training.real_ml_training_pipeline.psycopg2.connect')
...
class TestMLPipelinePerformance:
    ...
    def test_training_performance(self):
        ...
        training_data = pd.DataFrame({
            ...
            'won': np.random.choice([0, 1], n_samples, p=[0.85, 0.15])  # 15% win rate
        })
        ...
        if ML_PIPELINE_AVAILABLE:
            ...
            if features_df is not None:
                assert len(features_df) <= n_samples  # May drop some rows with missing values
