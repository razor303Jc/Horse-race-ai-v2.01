...
class TestMLModelTraining:
    ...
    def test_random_forest_training(self, mock_joblib_dump):
        ...
        if ML_PIPELINE_AVAILABLE:
            ...
            # Mock ML libraries
            with patch(
                "docker.ml_training.real_ml_training_pipeline.RandomForestClassifier"
            ) as mock_rf:
                with patch(
                    "docker.ml_training.real_ml_training_pipeline.train_test_split"
                ) as mock_split:
                    # Setup mocks
                    mock_model = Mock()
    ...
    def test_logistic_regression_training(self, mock_joblib_dump):
        ...
        if ML_PIPELINE_AVAILABLE:
            ...
            with patch(
                "docker.ml_training.real_ml_training_pipeline.LogisticRegression"
            ) as mock_lr:
                with patch(
                    "docker.ml_training.real_ml_training_pipeline.train_test_split"
                ) as mock_split:
