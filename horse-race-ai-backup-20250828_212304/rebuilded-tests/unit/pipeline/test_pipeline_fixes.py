...
class TestPipelineColumnMappings:
    ...
    def test_horses_id_mapping_fix(self):
        ...
        if "results_horses" in CSV_COLUMN_MAPPINGS:
            ...
            ), "CSV 'id' should map to DB 'horse_id'"
...
class TestPipelineIntegrationFixes:
    ...
    def test_complete_data_processing_workflow(self):
        ...
        records_clean = DataCleaner.clean_numeric_fields(
            records_clean, "finish_position"
        )
        records_clean = DataCleaner.clean_numeric_fields(records_clean, "age")

        # Percentage cleaning
        jockeys_clean = DataCleaner.clean_percentage_fields(
            jockeys_clean, "win_percentage"
        )
        ...
        assert pd.isna(
            records_clean["finish_position"].iloc[1]
        )  # Dash converted to NaN
    ...
    def test_critical_fixes_validation(self):
        ...
        if "jockeys_stats" in CSV_COLUMN_MAPPINGS:
            ...
            assert (
                "uptodate" in jockeys_mapping
            ), "Fix 2: jockeys uptodate mapping missing"
        ...
        if "trainers_stats" in CSV_COLUMN_MAPPINGS:
            ...
            assert (
                "uptodate" in trainers_mapping
            ), "Fix 3: trainers uptodate mapping missing"
        ...
        assert pd.isna(
            cleaned_df["test_field"].iloc[1]
        ), "Fix 4: dash cleaning not working"
...
class TestPipelinePerformance:
    ...
    def test_large_dataset_processing(self):
        ...
        large_df = pd.DataFrame(
            {
                ...
                "age": [
                    4 if i % 3 != 0 else "-" for i in range(12000)
                ],  # Mix valid and dash
        ...
        assert (
            processing_time < 5.0
        ), f"Large dataset processing too slow: {processing_time}s"
