...
from enhanced_ml_cycle_manager import DockerLoggingSetup, EnhancedMLCycleManager
...
class PipelineMLIntegrationManager:
    ...
    def _load_integration_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "max_training_time_minutes": int(
                os.getenv("MAX_TRAINING_TIME_MINUTES", 210)
            ),
            "enable_early_morning": os.getenv("ENABLE_EARLY_MORNING", "true").lower()
            == "true",
    ...
    async def initialize_components(self) -> bool:
        ...
        try:
            ...
            all_healthy = all(
                ...
                for key in ["pipeline", "ml_training", "database"]
            )
            ...
            if all_healthy:
                ...
                failed_components = [
                    ...
                    if not v and k != "last_check"
                ]
                logger.warning(
                    f"⚠️ Some components failed to initialize: {failed_components}"
                )
                return False
    ...
    async def get_current_pipeline_schedule(
        self, first_race_time: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"📋 Pipeline schedule loaded: {schedule.get('timing_analysis', {}).get('schedule_type', 'unknown')} mode"
            )
    ...
    async def execute_early_morning_training(self) -> Dict[str, Any]:
        ...
        try:
            ...
            training_start = datetime.strptime(
                self.early_morning_config["training_start"], "%H:%M"
            ).time()
            training_end = datetime.strptime(
                self.early_morning_config["training_end"], "%H:%M"
            ).time()
            ...
            if training_start <= current_time <= training_end:
                ...
                logger.warning(
                    f"⏰ Outside training window (current: {current_time}, window: {training_start}-{training_end})"
                )
                execution_results["errors"].append(
                    f"Outside training window: {current_time}"
                )
            ...
            execution_results["performance_metrics"] = {
                ...
                "training_efficiency": execution_results.get(
                    "training_results", {}
                ).get("efficiency", 0),
        ...
        try:
            ...
            try:
                if self.pipeline_allocator:
                    ...
                    test_schedule = self.pipeline_allocator.allocate_stage_times(
                        "00:01", "13:45"
                    )
            ...
            if all(status == "healthy" for status in component_statuses):
                ...
                health_report["recommendations"].append(
                    "Check error logs and restart failed components"
                )
            else:
