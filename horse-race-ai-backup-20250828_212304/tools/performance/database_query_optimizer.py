...
class DatabaseQueryOptimizer:
    ...
    async def initialize_database(self):
        ...
        try:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_query_metrics_timestamp ON query_performance_metrics(timestamp)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_query_metrics_hash ON query_performance_metrics(query_hash)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_slow_queries_timestamp ON slow_queries_log(timestamp)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_optimization_cache_hash ON query_optimization_cache(query_hash)"
            )
    ...
    def analyze_query(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis["optimization_recommendations"] = self._apply_optimization_rules(
                query
            )

            # Generate index recommendations
            analysis["index_recommendations"] = self._generate_index_recommendations(
                query
            )
            ...
            if rewritten != query:
                ...
                analysis["estimated_improvement"] = self._estimate_improvement(
                    query, rewritten
                )
    ...
    def _calculate_complexity_score(self, query: str) -> float:
        ...
        # Aggregations
        agg_count = len(
            re.findall(r"\b(count|sum|avg|max|min|group by)\b", query_lower)
        )
        score += agg_count * 10
    ...
    def _extract_join_columns(self, query: str) -> Dict[str, List[str]]:
        ...
        join_matches = re.findall(
            r"join\s+\w+.*?on\s+(\w+)\.(\w+)\s*=\s*(\w+)\.(\w+)", query, re.IGNORECASE
        )
    ...
    def _rewrite_query(self, query: str) -> str:
        ...
        if re.search(
            r"SELECT \* FROM (races|horses|records)", rewritten, re.IGNORECASE
        ):
        ...
        if (
            "SELECT" in rewritten.upper()
            and "LIMIT" not in rewritten.upper()
            and "COUNT(" not in rewritten.upper()
        ):
    ...
    async def execute_optimized_query(
        self, query: str, database_type: str = "sqlite", params: Optional[Tuple] = None
    ) -> Dict[str, Any]:
    ...
    async def _execute_postgres_query(
        ...
        async with self.postgres_pool.acquire() as conn:
            ...
            return {"data": [dict(row) for row in result], "rows_returned": len(result)}
        ...
        try:
            ...
            if execution_time > self.performance_thresholds["slow_query_threshold"]:
                await self._log_slow_query(
                    original_query, execution_time, database_type, analysis
                )
            ...
            for query_text, avg_time, frequency in slow_queries:
                ...
                for rec in query_recommendations:
                    recommendation = IndexRecommendation(
                        ...
                        * frequency,  # Estimate based on frequency
    ...
    async def _save_index_recommendations(
        self, recommendations: List[IndexRecommendation]
    ):
    ...
    async def create_recommended_indexes(
        ...
        if not auto_apply:
            return {
                "message": "Auto-apply is disabled. Review recommendations manually."
            }
        ...
        try:
            ...
            for table_name, column_names, index_type, improvement in recommendations:
                try:
                    ...
                    logger.warning(
                        f"Error creating index for {table_name}.{column_names}: {e}"
                    )
            ...
            return {
                ...
                "total_estimated_improvement": sum(
                    idx["estimated_improvement"] for idx in created_indexes
                ),
            }
        ...
        try:
            ...
            if unoptimized_time > 0 and optimized_time > 0:
                optimization_benefit = (
                    (unoptimized_time - optimized_time) / unoptimized_time
                ) * 100
            ...
            return {
                ...
                "performance_metrics": {
                    ...
                    "optimization_rate": round(
                        (perf_summary[2] or 0) / max(perf_summary[0] or 1, 1) * 100, 1
                    ),
                ...
                "index_recommendations": {
                    ...
                    "application_rate": round(
                        (index_summary[1] or 0) / max(index_summary[0] or 1, 1) * 100, 1
                    ),
...
async def main():
    ...
    try:
        ...
        for i, query in enumerate(test_queries, 1):
            ...
            print(
                f"   Index Suggestions: {len(analysis.get('index_recommendations', []))}"
            )
        ...
        for rec in recommendations[:3]:  # Show first 3
            print(
                f"   • {rec.table_name}.{','.join(rec.column_names)} ({rec.index_type})"
            )
        ...
        print(
            f"   - Optimization benefit: {perf_metrics['optimization_benefit_percent']}%"
        )
