...
class SimplifiedParallelTrainer:
    ...
    def _engineer_comprehensive_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_features["win_odds"] = pd.to_numeric(
            df_features["win_odds"], errors="coerce"
        )
        df_features["place_odds"] = pd.to_numeric(
            df_features["place_odds"], errors="coerce"
        )
        ...
        df_features["horse_weight_kg"] = pd.to_numeric(
            df_features["horse_weight_kg"], errors="coerce"
        )
        df_features["handicap_weight"] = pd.to_numeric(
            df_features["handicap_weight"], errors="coerce"
        )
        ...
        place_odds_safe = (
            df_features["place_odds"].fillna(df_features["win_odds"]).clip(lower=0.1)
        )
        df_features["odds_ratio"] = df_features["win_odds"] / place_odds_safe

        weight_denom = (
            df_features["handicap_weight"]
            .fillna(df_features["horse_weight_kg"])
            .clip(lower=30)
        )
        ...
        df_features["jockey_win_pct"] = pd.to_numeric(
            df_features["jockey_win_pct"], errors="coerce"
        ).fillna(10)
        df_features["jockey_place_pct"] = pd.to_numeric(
            df_features["jockey_place_pct"], errors="coerce"
        ).fillna(25)
        df_features["trainer_win_pct"] = pd.to_numeric(
            df_features["trainer_win_pct"], errors="coerce"
        ).fillna(10)
        df_features["trainer_place_pct"] = pd.to_numeric(
            df_features["trainer_place_pct"], errors="coerce"
        ).fillna(25)
        ...
        df_features["combined_place_pct"] = (
            df_features["jockey_place_pct"] + df_features["trainer_place_pct"]
        ) / 2
        ...
        logger.info(
            f"✅ Engineered features: {len([c for c in df_features.columns if c not in df.columns])} new features"
        )
    ...
    def train_single_model(self, args: Tuple) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"✅ {model_name}: AUC={test_metrics['roc_auc']:.4f}, CV={grid_search.best_score_:.4f}"
            )
            return result
    ...
    def train_models_parallel(
        ...
        logger.info(
            f"📊 Training: {len(X_train):,} samples, Test: {len(X_test):,} samples"
        )
        ...
        with ProcessPoolExecutor(
            max_workers=min(len(model_configs), os.cpu_count())
        ) as executor:
            future_to_model = {
    ...
    def create_ensemble_model(
        ...
        top_models = sorted(
            [
                (name, perf["test_metrics"]["roc_auc"])
        ...
        logger.info(
            f"🎯 Using top 3 models for ensemble: {[name for name, _ in top_models]}"
        )
    ...
    def save_models_and_results(self, training_results: Dict, ensemble_result: Dict):
        ...
        metadata = {
            ...
            "model_performance": {
                name: {
                    k: v for k, v in perf.items() if k not in ["best_model", "scaler"]
                }
                for name, perf in self.model_performance.items()
            },
    ...
    def print_results_summary(self, training_results: Dict, ensemble_result: Dict):
        ...
        if successful:
            ...
            for i, (name, result) in enumerate(sorted_models, 1):
                ...
                print(
                    f"   {i}. {name:20}: AUC={metrics['roc_auc']:.4f}, "
        ...
        if ensemble_result:
            ...
            print(
                f"   Voting Ensemble: AUC={metrics['roc_auc']:.4f}, "
                f"Acc={metrics['accuracy']:.4f}"
            )
        ...
        (
            ...
            if ensemble_result
            else print(f"   ⚠️ No ensemble created")
        )
    ...
    def run_complete_pipeline(self) -> Dict:
        ...
        try:
            ...
            ensemble_result = self.create_ensemble_model(
