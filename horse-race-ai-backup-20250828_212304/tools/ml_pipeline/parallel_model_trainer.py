...
class ParallelModelTrainer:
    ...
    def load_preprocessed_data(self) -> Tuple[np.ndarray, np.ndarray, List[str]]:
        ...
        try:
            ...
            pipeline_path = (
                self.project_root / "trained_models" / "ml_preprocessing_pipeline.pkl"
            )

            if not pipeline_path.exists():
                raise FileNotFoundError(
                    "ML preprocessing pipeline not found. Run Priority 1B first."
                )
            ...
            logger.info(
                f"✅ Features processed: {X_processed.shape[1]} features, {len(y):,} samples"
            )
    ...
    def _engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_features[numeric_columns] = df_features[numeric_columns].fillna(
            df_features[numeric_columns].median()
        )
    ...
    def train_single_model(
        self, model_config: Tuple[str, Dict, np.ndarray, np.ndarray]
    ) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"✅ {model_name}: CV AUC={result['cv_mean']:.4f}±{result['cv_std']:.4f}, Time={training_time:.1f}s"
            )
            return result
    ...
    def train_models_parallel(
        ...
        logger.info(
            "🚀 Starting parallel model training with hyperparameter optimization..."
        )
        ...
        training_args = [
            (name, config, X_train, y_train) for name, config in model_configs.items()
        ]
        ...
        for result in training_results:
            try:
                ...
                logger.error(
                    f"❌ Test evaluation failed for {result['model_name']}: {e}"
                )
                failed_models.append(
                    {"model_name": result["model_name"], "error": str(e)}
                )
        ...
        if failed_models:
            logger.warning(
                f"⚠️ {len(failed_models)} models failed: {[m['model_name'] for m in failed_models]}"
            )
    ...
    def create_ensemble_models(
        ...
        ensemble_configs = [
            ...
            {
                "name": "top_3_hard_voting",
                "models": model_rankings[:3],
                "voting": "hard",
            },
        ...
        for config in ensemble_configs:
            try:
                ...
                estimators = [
                    (name, self.trained_models[name]) for name, _ in config["models"]
                ]

                # Create voting classifier
                ensemble = VotingClassifier(
                    estimators=estimators, voting=config["voting"]
                )
                ...
                logger.info(
                    f"✅ {ensemble_name}: CV AUC={cv_scores.mean():.4f}±{cv_scores.std():.4f}, "
    ...
    def print_comprehensive_summary(
        self, training_results: Dict, ensemble_results: Dict
    ):
        ...
        if training_results["successful_models"]:
            ...
            print(
                f"   🥇 Best individual model: {best_individual[0]} "
        ...
        if ensemble_results:
            ...
            print(
                f"   🏆 Best ensemble model: {best_ensemble[0]} "
    ...
    def run_complete_parallel_training(self) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"🎉 Priority 3A: Parallel Model Training completed in {total_time:.1f} seconds"
            )
