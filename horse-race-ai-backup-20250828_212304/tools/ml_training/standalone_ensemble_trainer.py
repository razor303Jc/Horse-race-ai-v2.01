#!/usr/bin/env python3
"""
Standalone Ensemble Training Script for Results Database
======================================================

Trains ensemble models directly on actual race results without external dependencies.
Optimizes ensemble weighting using the 388 race records with actual positions.
"""
...
class StandaloneEnsembleTrainer:
    """Standalone ensemble trainer for results database."""

    def __init__(self, models_dir: str = "/app/models"):
        """Initialize the standalone trainer."""
    ...
    def engineer_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
        ...
        features_df["draw_percentile"] = race_groups["draw"].rank(pct=True)
    ...
    def train_optimized_ensemble(self, save_model: bool = True) -> Dict[str, Any]:
        ...
        final_model_data = {
            ...
            "training_metadata": {
                ...
                "model_version": "v2.04_standalone_optimized",
            },
        }
    ...
    def save_optimized_model(self, model_data: Dict[str, Any]) -> str:
        ...
        model_filename = f"standalone_optimized_ensemble_{timestamp}.joblib"
        model_path = self.models_dir / model_filename

        joblib.dump(model_data, model_path)

        # Also save a latest version
        latest_path = self.models_dir / "standalone_optimized_ensemble_latest.joblib"
    ...
    def generate_training_summary(self, model_data: Dict[str, Any]) -> None:
        ...
        logger.info("\n" + "=" * 60)
        logger.info("🏆 STANDALONE OPTIMIZED ENSEMBLE TRAINING COMPLETE")
        logger.info("=" * 60)
...
def main():
    ...
    logger.info("🏇 Starting Standalone Ensemble Training with Results Database...")

    try:
        # Initialize trainer
        trainer = StandaloneEnsembleTrainer()

        # Train optimized ensemble
        model_data = trainer.train_optimized_ensemble(save_model=True)

        if model_data:
            logger.info("✅ Standalone ensemble training completed successfully!")
            return model_data
        else:
