...
class TimeAwareMLOptimizer:
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    "horse_racing_db",
                    "-c",
                    "SELECT MIN(race_time) FROM races WHERE race_date = CURRENT_DATE;",
                ],
                capture_output=True,
                text=True,
            )
                    ...
                    if ":" in line and len(line.strip()) > 0:
                        try:
                            ...
                            race_time = datetime.strptime(
                                f"{today} {race_time_str}", "%Y-%m-%d %H:%M:%S"
                            )
                            return race_time
                        except ValueError:
                            continue
    ...
    def calculate_available_training_time(self) -> int:
        ...
        logger.info(
            f"Available training time: {available_minutes} minutes until {deadline}"
        )
    ...
    def can_start_new_cycle(self) -> bool:
        ...
        logger.info(
            f"Available: {available_time}min, Estimated cycle: {estimated_cycle_time}min"
        )
    ...
    def run_training_cycle(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"✅ Cycle {self.current_cycle + 1} completed in {cycle_results['duration_minutes']:.1f} minutes"
            )
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    method,
                    "--cycle",
                    str(self.current_cycle + 1),
                    (
                        "--optimize-hyperparameters"
                        if self.training_config.get("hyperparameter_search")
                        else "--no-optimize"
                    ),
                    "--output-dir",
                    str(self.models_dir),
    ...
    def _optimize_ensemble_weights(self, models: Dict[str, Any]) -> Dict[str, float]:
        ...
        for method, model_result in models.items():
            if (
                "performance_metrics" in model_result
                and model_result["performance_metrics"]
            ):
                # Use F1 score as primary metric, fallback to accuracy
                score = model_result["performance_metrics"].get(
                    "f1_score", model_result["performance_metrics"].get("accuracy", 0)
                )
            else:
                score = 0
    ...
    def _evaluate_ensemble(
        self, models: Dict[str, Any], weights: Dict[str, float]
    ) -> Dict[str, float]:
        ...
        for metric in metrics_to_aggregate:
            ...
            for method, model_result in models.items():
                if (
                    "performance_metrics" in model_result
                    and metric in model_result["performance_metrics"]
                ):
    ...
    def _analyze_performance_and_suggest_optimizations(
    ...
    def save_cycle_results(self, cycle_results: Dict[str, Any]):
        ...
        if self.best_performance is None or current_f1 > self.best_performance.get(
            "f1_score", 0
        ):
        ...
        # Save to file
        results_file = (
            self.results_dir / f"training_cycle_{cycle_results['cycle']}.json"
        )
        ...
        summary = {
            ...
            "performance_history": [
                r["ensemble_performance"] for r in self.performance_history
            ],
            "training_start": (
            ...
            "first_race_time": (
                self.first_race_time.isoformat() if self.first_race_time else None
            ),
        }
...
╰─────────────────────────────────────────╯
        ...
        try:
            ...
            min_training_time = self.training_config.get(
                "minimum_training_time_minutes", 120
            )
            ...
            if available_time < min_training_time:
                logger.warning(
                    f"⚠️  Only {available_time} minutes available, less than minimum {min_training_time} minutes"
                )
                final_results["time_constrained"] = True

            # Run training cycles
            while self.current_cycle < max_cycles and self.can_start_new_cycle():
                ...
                if "optimizations" in cycle_results and not cycle_results[
                    "optimizations"
                ].get("continue_training", True):
                ...
                logger.info(
                    f"📊 Cycle {self.current_cycle} complete. Time remaining: {remaining_time} minutes"
                )

            # Final summary
            if self.performance_history:
                final_results["training_completed"] = True
                final_results["final_performance"] = self.performance_history[-1][
                    "ensemble_performance"
                ]
                ...
                for i in range(1, len(self.performance_history)):
                    prev_f1 = self.performance_history[i - 1][
                        "ensemble_performance"
                    ].get("f1_score", 0)
                    curr_f1 = self.performance_history[i]["ensemble_performance"].get(
                        "f1_score", 0
                    )
                ...
                final_results["optimization_summary"] = {
                    ...
                    "average_cycle_improvement": (
                        np.mean(improvements) if improvements else 0
                    ),
                    "best_cycle": np.argmax(
                        [
                            r["ensemble_performance"].get("f1_score", 0)
                            for r in self.performance_history
                        ]
                    )
                    + 1,
                    "training_efficiency": (
                        final_results["best_performance"].get("f1_score", 0)
                        / total_time
                        if total_time > 0
                        else 0
                    ),
...
def main():
    ...
    parser.add_argument(
        "--first-race-time", default=None, help="First race time (HH:MM)"
    )
    parser.add_argument(
        "--max-cycles", type=int, default=5, help="Maximum training cycles"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Show time calculations only"
    )
    ...
    if args.first_race_time:
        try:
            today = datetime.now().date()
            race_time = datetime.strptime(
                f"{today} {args.first_race_time}", "%Y-%m-%d %H:%M"
            )
