...
class CyclicMLTrainer:
    ...
    def train_single_cycle(self, cycle_num, session_num):
        ...
        models = {
            ...
            "LogisticRegression": LogisticRegression(
                C=1.0 + (cycle_num * 0.1), random_state=random_state, max_iter=1000
            ),
        }
        ...
        best_model_name = max(
            cycle_results.keys(), key=lambda x: cycle_results[x]["auc"]
        )
        ...
        self.performance_trends["accuracy_scores"].append(
            cycle_results[best_model_name]["accuracy"]
        )
        self.performance_trends["cycle_times"].append(cycle_time)

        # Update best models if performance improved
        if (
            best_model_name not in self.best_models
            or best_auc > self.best_models[best_model_name]["auc"]
        ):
        ...
        logger.info(
            f"  ✅ Best: {best_model_name} - AUC: {best_auc:.4f}, Accuracy: {cycle_results[best_model_name]['accuracy']:.4f}"
        )
    ...
    def save_training_session(self, session_name):
        ...
        logger.info(
            f"  Best AUC achieved: {max(self.performance_trends['auc_scores']):.4f}"
        )
        logger.info(
            f"  Average AUC: {np.mean(self.performance_trends['auc_scores']):.4f}"
        )
    ...
    def run_training_cycles(
        ...
        logger.info(
            f"🚀 Starting {num_cycles} cycles with {sessions_per_cycle} sessions each"
        )
        ...
        for cycle in range(1, num_cycles + 1):
            ...
            if cycle % 5 == 0 or cycle == num_cycles:
                ...
                if isinstance(analysis, dict):
                    ...
                    logger.info(
                        f"  Avg Cycle Time: {analysis['average_cycle_time']:.2f}s"
                    )
            ...
            logger.info(
                f"🎯 Progress: {cycle}/{num_cycles} cycles, {total_sessions} total sessions"
            )
        ...
        logger.info(
            f"  Best overall AUC: {max(self.performance_trends['auc_scores']):.4f}"
        )
        logger.info(f"  Best models: {list(self.best_models.keys())}")
...
def main():
    ...
    phase1_analysis = trainer.run_training_cycles(
        num_cycles=5, sessions_per_cycle=5, session_name="phase1_5x5", wait_time=2
    )
    ...
    phase2_analysis = trainer.run_training_cycles(
        num_cycles=10, sessions_per_cycle=10, session_name="phase2_10x10", wait_time=1
    )
