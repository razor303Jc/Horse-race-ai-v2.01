...
- GET /api/daily_races         # DailyRaces component data
- GET /api/real_race_cards     # RealRaceCards component data
- GET /api/race_details/{race_id}  # Race card details
...
- GET /api/betting/recommendations  # Betting recommendations
- GET /api/stage8/performance  # Stage 8 performance data
"""
...
class APIExtensions:
    ...
    def _setup_extended_routes(self):
        ...
        async def get_live_races(request: Request):
            ...
            try:
                ...
                ) @ self.app.get("/api/races/schedule")

        @self.limiter.limit("30/minute")
        async def get_race_schedule(request: Request, date: Optional[str] = None):
            """Get daily race schedule."""
            try:
                target_date = date or datetime.now().date().isoformat()
                schedule = await self._get_race_schedule(target_date)
                return {
                    "status": "success",
                    "date": target_date,
                    "schedule": schedule,
                ...
                raise HTTPException(
                    status_code=500, detail="Failed to fetch race schedule"
                )
        ...
        @self.app.get("/api/betting/portfolio")
        async def get_betting_portfolio(
            current_user: dict = Depends(self.get_current_user),
        ):
            """Get user's betting portfolio."""
            try:
                portfolio = await self._get_user_betting_portfolio(
                ...
                raise HTTPException(
                    status_code=500, detail="Failed to fetch betting portfolio"
                )

        @self.app.post("/api/betting/place")
        async def place_bet(
            bet_data: BetPlacement, current_user: dict = Depends(self.get_current_user)
        ):
            """Place a new bet."""
            try:
                result = await self._place_bet(bet_data, current_user["user_id"])
                return result
            except Exception as e:
                logger.error(f"Error placing bet: {e}")
                raise HTTPException(status_code=500, detail="Failed to place bet")

        @self.app.get("/api/user/preferences")
        async def get_user_preferences(
            current_user: dict = Depends(self.get_current_user),
        ):
            """Get user preferences."""
            try:
                preferences = await self._get_user_preferences(current_user["user_id"])
                return preferences
            except Exception as e:
        ...
        @self.app.put("/api/user/preferences")
        async def update_user_preferences(
            ...
            try:
                result = await self._update_user_preferences(
        ...
        async def get_betting_recommendations(request: Request):
            ...
            try:
                ...
                logger.error(f"Error fetching betting recommendations: {e}")
                raise HTTPException(
        ...
        async def get_stage8_performance(request: Request):
            ...
            try:
                ...
                logger.error(f"Error fetching Stage 8 performance: {e}")
                raise HTTPException(
    ...
    async def _get_live_races(self) -> List[Dict]:
        ...
        # Implementation will connect to your racing database
    ...
    async def _get_race_schedule(self, date: str) -> List[Dict]:
        """Get race schedule for specific date."""
        # Implementation will query your database for race schedule
        return [
            {
                "race_id": "schedule_001",
                "venue": "Cheltenham",
    ...
    async def _get_daily_races_comprehensive(self) -> Dict:
        """Get comprehensive daily races data matching DailyRaces component expectations."""
        # This should match the DailyRacesStats interface from the React component
        return {
            "total_races": 45,
            "total_meetings": 8,
            "daily_stats": {
            ...
            "races": [
                {
                    ...
                    "race_insights": [
                        ...
                        "Several horses dropping in class",
                    ],
                }
            ],
        }
    ...
    async def _get_real_race_cards(self) -> Dict:
        """Get real race cards data matching RealRaceCards component expectations."""
        return {
            "total_races": 32,
            "total_horses": 384,
    ...
    async def _get_race_card_details(self, race_id: str) -> Dict:
        """Get detailed race card information matching React component expectations."""
        return {
            "race_id": race_id,
            "race_name": "Class 2 Handicap",
            "time": "15:30",
    ...
    async def _get_user_betting_portfolio(self, user_id: int) -> Dict:
        """Get user's betting portfolio."""
        return {
            "account_balance": 1250.00,
            "total_invested": 5000.00,
            "current_value": 5375.00,
            "total_return": 375.00,
            "roi_percentage": 7.5,
            "active_bets": 3,
            "settled_bets": 47,
    ...
    async def _place_bet(self, bet_data: BetPlacement, user_id: int) -> Dict:
        """Place a new bet."""
        # Implementation would integrate with betting system
        bet_id = f"bet_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{user_id}"
        ...
        return {
            ...
            "bet_details": {
                ...
                "odds": bet_data.odds or 3.5,
                "potential_return": bet_data.stake * (bet_data.odds or 3.5),
                "timestamp": datetime.now().isoformat(),
            },
        }
    ...
    async def _get_user_preferences(self, user_id: int) -> Dict:
        ...
        return {
            ...
            "favorite_tracks": ["Cheltenham", "Newmarket", "Ascot"],
        }
    ...
    async def _get_performance_analytics(self, user_id: int) -> Dict:
        """Get performance analytics."""
        return {
            "overall_performance": {
                "total_bets": 156,
                ...
                "win_rate": 57.1,
                "total_staked": 7800.0,
                "total_returns": 8542.0,
                "net_profit": 742.0,
                "roi": 9.5,
            },
            "monthly_performance": [
                {"month": "2025-01", "profit": 125.0, "roi": 12.5},
                {"month": "2025-02", "profit": 87.0, "roi": 8.7},
                {"month": "2025-03", "profit": 156.0, "roi": 15.6},
            ],
            "performance_by_track": [
                {"track": "Cheltenham", "bets": 34, "profit": 245.0, "roi": 18.2},
                {"track": "Newmarket", "bets": 28, "profit": 156.0, "roi": 12.1},
            ],
        }
