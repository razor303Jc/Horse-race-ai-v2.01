        ...
        try:
            if self.service_pid_file.exists():
                ...
                if psutil.pid_exists(pid):
                    ...
                    if "uvicorn" in process.name() or "prediction_api" in " ".join(
                        process.cmdline()
                    ):
                        return True
    ...
    def start_prediction_service(self) -> bool:
        ...
        try:
            ...
            if self.verify_service_health():
                logger.info(
                    f"✅ Prediction service started successfully (PID: {process.pid})"
                )
                return True
            else:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"✅ Service healthy: {health_data.get('status', 'unknown')}"
                    )
                    return True
                ...
                if attempt < max_attempts - 1:
                    logger.info(
                        f"⏳ Health check attempt {attempt + 1}/{max_attempts}, retrying..."
                    )
                    time.sleep(2)
                else:
    ...
    def test_prediction_endpoint(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"✅ Prediction test successful: {prediction.get('confidence', 'N/A')}"
                )
                return True
            else:
    ...
    def run_phase6_pipeline(self) -> bool:
        ...
        try:
            ...
            if not self.test_prediction_endpoint():
                logger.warning(
                    "⚠️ Prediction test failed, but service appears to be running"
                )
            ...
            logger.info(
                "✅ Phase 6: Prediction service activation completed successfully"
            )
            logger.info(
                f"🌐 Prediction API available at: http://localhost:{self.api_port}"
            )
            logger.info(
                f"📖 API documentation at: http://localhost:{self.api_port}/docs"
            )
            return True
