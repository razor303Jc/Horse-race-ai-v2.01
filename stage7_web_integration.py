...
class Stage7WebIntegration:
    ...
    def check_web_app_status(self) -> bool:
        ...
        try:
            ...
            if "unhealthy" in result.stdout.lower():
                logger.warning(
                    "⚠️ Web container marked as unhealthy, attempting to restart..."
                )
    ...
    def restart_web_container(self) -> bool:
        ...
        try:
            ...
            subprocess.run(
                ["docker", "restart", self.web_container], check=True, timeout=30
            )
        ...
        try:
            ...
            health_response = subprocess.run(
                [
                    ...
                    "curl",
                    "-s",
                    f"http://localhost:{self.prediction_api_port}/health",
                ],
                capture_output=True,
                text=True,
                timeout=10,
    ...
    def setup_real_time_updates(self) -> bool:
        ...
        try:
            ...
            realtime_config = {
                "websocket": {"enabled": True, "port": 8001, "path": "/ws"},
                "polling": {
                    "enabled": True,
                    "interval": 30,  # seconds
                    "endpoints": ["/models/status", "/health"],
                },
                "notifications": {
                    "predictions": True,
                    "system_status": True,
    ...
    def generate_stage7_summary(self) -> Dict:
        ...
        summary = {
            ...
            "status": (
                "operational" if all(self.integration_status.values()) else "partial"
            ),
            "timestamp": datetime.now().isoformat(),
