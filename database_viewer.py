    ...
    with sqlite3.connect(db_path) as conn:
        ...
        if daily_data:
            ...
            for row in daily_data:
                date, downloads, files, size_bytes, races, horses, jockeys, trainers, records, status = row
        ...
        if today_downloads:
            ...
            if cards_data:
                ...
                for download in cards_data:
                    timestamp, source_type, category, filename, size_bytes, records, status, archive_id = download
                    size_kb = size_bytes / 1024
                    
                    status_icon = "📦" if status == "archived" else "📁"
                    archive_text = f" (Archive: {archive_id})" if archive_id else ""
                    
                    print(f"   {status_icon} {category}/{filename}: {records or 0} records ({size_kb:.1f} KB){archive_text}")
    ...
    with sqlite3.connect(archive_db_path) as conn:
        ...
        if archives:
            for archive in archives:
                id, date, filename, files, size_bytes, compression_ratio, timestamp, status = archive
