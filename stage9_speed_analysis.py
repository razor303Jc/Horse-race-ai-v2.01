...
class SpeedAnalysisEngine:
    ...
    def _initialize_speed_standards(self) -> Dict:
        ...
        return {
            "5f": {"class_1": 58.0, "class_2": 59.0, "class_3": 60.0, "class_4": 61.0},
            "6f": {"class_1": 70.0, "class_2": 71.5, "class_3": 73.0, "class_4": 74.5},
            "7f": {"class_1": 82.0, "class_2": 84.0, "class_3": 86.0, "class_4": 88.0},
            "1m": {
                "class_1": 96.0,
                "class_2": 98.5,
                "class_3": 101.0,
    ...
    async def analyze_race_speeds(
        self, race_data: pd.DataFrame
    ) -> SpeedAnalysisResults:
        ...
        try:
            ...
            for race_id in unique_races:
                ...
                logger.info(
                    f"📊 Analyzing race {race_id} with {len(race_horses)} horses"
                )

                # Calculate speed figures for each horse
                race_speed_figures = await self._calculate_race_speed_figures(
                    race_horses, race_id
                )
                ...
                pace_prediction = self._predict_pace_scenario(
                    pace_analysis, race_running_styles
                )
            ...
            logger.info(
                f"📈 Generated {len(speed_figures)} speed figures, {len(pace_analyses)} pace analyses"
            )
    ...
    async def _calculate_race_speed_figures(
        self, race_horses: pd.DataFrame, race_id: str
    ) -> List[SpeedFigure]:
        ...
        for _, horse in race_horses.iterrows():
            try:
                ...
                if self.config["speed_calculation"]["weight_adjustment"]:
                    adjusted_speed = self._apply_weight_adjustment(
                        adjusted_speed, weight
                    )

                if self.config["speed_calculation"]["track_condition_adjustment"]:
                    adjusted_speed = self._apply_track_condition_adjustment(
                        adjusted_speed, track_condition
                    )

                if self.config["speed_calculation"]["class_adjustment"]:
                    adjusted_speed = self._apply_class_adjustment(
                        adjusted_speed, race_class
                    )
                ...
                speed_map_position = self._calculate_speed_map_position(
                    horse, race_horses
                )
                ...
                logger.warning(
                    f"⚠️ Failed to calculate speed figure for {horse_name}: {e}"
                )
                continue

        return speed_figures

    async def _analyze_race_pace(
        self, race_horses: pd.DataFrame, race_id: str
    ) -> PaceAnalysis:
        ...
        try:
            # Get race distance
            distance = (
                race_horses["distance"].iloc[0]
                if "distance" in race_horses.columns
                else 6.0
            )
            ...
            if all_sectionals:
                ...
                mid_pace = (
                    np.mean(avg_sectionals[1:-1]) if len(avg_sectionals) > 2 else 25.0
                )
            ...
            pace_scenario = self._determine_pace_scenario(
                early_pace, mid_pace, late_pace, distance
            )
            ...
            return PaceAnalysis(
                ...
                sectional_splits=(
                    avg_sectionals.tolist() if "avg_sectionals" in locals() else []
                ),
                speed_map=speed_map,
                pace_bias=pace_bias,
                calculated_at=datetime.now().isoformat(),
    ...
    async def _classify_running_styles(
        self, race_horses: pd.DataFrame
    ) -> List[RunningStyle]:
        ...
        for _, horse in race_horses.iterrows():
            try:
                ...
                style, confidence = self._determine_running_style(
                    early_avg, mid_avg, late_avg
                )
    ...
    def _calculate_base_speed_figure(
        self, distance: float, time_seconds: float
    ) -> float:
    ...
    def _apply_track_condition_adjustment(
        self, speed_figure: float, condition: str
    ) -> float:
    ...
    def _generate_sectional_times(
        self, distance: float, total_time: float
    ) -> List[float]:
    ...
    def _classify_pace_style(
        self, sectional_times: List[float], distance: float
    ) -> str:
    ...
    def _calculate_speed_map_position(
        self, horse: pd.Series, all_horses: pd.DataFrame
    ) -> int:
        """Calculate position in speed map (1-12)"""
        # Simplified speed map calculation
        time_seconds = horse.get("time_seconds", 75.0)
        all_times = (
            all_horses["time_seconds"]
            if "time_seconds" in all_horses.columns
            else [75.0] * len(all_horses)
        )
    ...
    def _determine_pace_scenario(
        self, early: float, mid: float, late: float, distance: float
    ) -> str:
            ...
            speed_map.append(
                {
                    ...
                    "predicted_style": (
                        "front_runner"
                        if early_speed <= 3
                        else "closer" if early_speed >= 10 else "presser"
                    ),
                }
            )
    ...
    def _determine_pace_bias(
        self, speed_map: List[Dict], sectionals: List[List[float]]
    ) -> str:
    ...
    def _generate_speed_ratings(
        self, speed_figures: List[SpeedFigure]
    ) -> Dict[str, float]:
    ...
    def _predict_pace_scenario(
        self, pace_analysis: PaceAnalysis, running_styles: List[RunningStyle]
    ) -> str:
        """Predict the likely pace scenario for the race"""
        front_runners = sum(
            1 for style in running_styles if style.primary_style == "front_runner"
        )
    ...
    def _determine_running_style(
        self, early_avg: float, mid_avg: float, late_avg: float
    ) -> Tuple[str, float]:
    ...
    async def save_results(
        self, results: SpeedAnalysisResults, output_path: Optional[Path] = None
    ) -> bool:
        ...
        return {
            **self.metrics,
            ...
            "figures_per_calculation": (
                self.metrics["speed_figures_generated"]
                / max(self.metrics["calculations_performed"], 1)
            ),
        }
...
class Stage9SpeedAnalysisOrchestrator:
    ...
    def __init__(self):
        ...
        (self.project_root / "data" / "speed_analysis").mkdir(
    ...
    def _generate_stage_summary(
        self, results: SpeedAnalysisResults, processing_time: float
    ) -> Dict[str, Any]:
        ...
        return {
            "execution_summary": {
                ...
                "average_speed_figure": (
                    round(np.mean([sf.speed_figure for sf in results.speed_figures]), 1)
                    if results.speed_figures
                    else 0
                ),
            },
            "pace_analysis_summary": {
                "pace_scenarios": list(
                    set([pa.pace_scenario for pa in results.pace_analyses])
                ),
                "pace_biases": list(
                    set([pa.pace_bias for pa in results.pace_analyses])
                ),
                "average_early_pace": (
                    round(np.mean([pa.early_pace for pa in results.pace_analyses]), 2)
                    if results.pace_analyses
                    else 0
                ),
            },
            "running_style_summary": {
                "style_distribution": self._count_running_styles(
                    results.running_styles
                ),
                "average_confidence": (
                    round(
                        np.mean([rs.style_confidence for rs in results.running_styles]),
                        2,
                    )
                    if results.running_styles
                    else 0
                ),
            },
            "stage_performance": {
                "within_time_limit": processing_time
                <= (self.stage_info["duration_minutes"] * 60),
                "efficiency_ratio": (
                    round(
                        (self.stage_info["duration_minutes"] * 60) / processing_time, 2
                    )
                    if processing_time > 0
                    else 0
                ),
                "data_throughput": (
                    round(results.total_horses / processing_time, 2)
                    if processing_time > 0
                    else 0
                ),
            },
    ...
    def _count_running_styles(
        self, running_styles: List[RunningStyle]
    ) -> Dict[str, int]:
    ...
    try:
        ...
        if results["success"]:
            ...
            if "execution_summary" in summary:
                ...
                print(
                    f"⚡ Speed Figures Generated: {exec_sum['speed_figures_generated']}"
                )
                print(
                    f"🏃 Pace Analyses Completed: {exec_sum['pace_analyses_completed']}"
                )
            ...
            if "stage_performance" in summary:
                ...
                print(
                    f"🎯 Within Time Limit: {'✅' if perf['within_time_limit'] else '❌'}"
                )
