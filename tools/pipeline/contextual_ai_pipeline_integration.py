...
class ContextualAIPipelineIntegration:
    ...
    async def run_contextual_ai_stage(
        ...
        try:
            ...
            if self.save_detailed_analysis:
                await self._save_detailed_analysis(
                    contextual_analysis, race_data
                )
            ...
            stage_results = {
                ...
                'stage_metadata': {
                    ...
                    'value_opportunities': len(
                        contextual_analysis.get('value_assessment', {})
                        .get('value_horses', [])
                    ),
    ...
    def _validate_input_data(self, pipeline_data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if not isinstance(race_data, dict) or not race_data:
                return {
                    'valid': False,
                    'error': "Invalid or empty race_data"
                }
            ...
            if not isinstance(horses_data, list) or len(horses_data) == 0:
                return {
                    'valid': False,
                    'error': "Invalid or empty horses_data"
                }
            
            # Check for required race fields
            required_race_fields = ['race_id']
            missing_race_fields = [
                field for field in required_race_fields 
    ...
    def _validate_analysis_results(self, analysis_results: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            required_sections = [
                ...
                'intelligent_alerts', 'confidence_analysis'
            ]
            ...
            if data_quality_score < self.min_data_quality_score:
                self.logger.warning(
                    f"Low data quality score: {data_quality_score}"
                )
            ...
            if overall_confidence < self.min_confidence_threshold:
                self.logger.warning(
                    f"Low analysis confidence: {overall_confidence}"
                )
            ...
            return {
                'valid': False,
                'error': f"Analysis validation error: {str(e)}"
            }
            ...
            for horse in horses_data:
                ...
                if horse_name in predictions:
                    ...
                    enriched_horse['implied_odds'] = (
                        ...
                        if horse_predictions.get('win_probability', 0) > 0 else 10.0
                    )
                    
                    enriched_horse['value_ratio'] = (
                        horse.get('odds', 5.0) / enriched_horse['implied_odds']
                        if enriched_horse['implied_odds'] > 0 else 1.0
                    )
    ...
    async def _process_contextual_results(
        ...
        try:
            ...
            processed_results['processing_metadata'] = {
                ...
                'analysis_duration': contextual_analysis.get('analysis_duration_seconds'),
                'contextual_factors_count': len(
                    contextual_analysis.get('race_conditions', {})
                ),
                'insights_generated': len(
                    contextual_analysis.get('form_insights', {})
                    .get('individual_insights', [])
                ),
                'patterns_identified': len(
                    contextual_analysis.get('pattern_analysis', {})
                    .get('emerging_patterns', [])
                )
            }
    ...
    def _generate_alerts_and_recommendations(
        ...
        try:
            ...
            alerts_and_recs['strategic_recommendations'] = [
                {
                    'approach': contextual_summary.get('recommended_approach', 'Standard'),
                    'confidence_level': contextual_summary.get('confidence_level', 'MEDIUM'),
                    'value_outlook': contextual_summary.get('value_outlook', 'Moderate'),
    ...
    async def _save_detailed_analysis(
        ...
        try:
            output_dir = self.config.get(
                'output_settings', {}
            ).get('output_directory', 'data/contextual_ai_analysis')
    ...
    def _merge_with_pipeline_data(
        ...
        try:
            ...
            enhanced_data['pipeline_stages']['contextual_ai_enhancement'] = stage_results
            ...
            enhanced_data['pipeline_metadata']['contextual_ai'] = {
                ...
                'analysis_quality': stage_results.get('analysis_summary', {})
                    .get('analysis_quality', 'unknown'),
                'alerts_count': stage_results.get('stage_metadata', {})
                    .get('alerts_generated', 0)
            }
...
async def main():
    ...
    print(f"Stage Status: {results.get('pipeline_stages', {}).get('contextual_ai_enhancement', {}).get('status', 'Unknown')}")
    
    stage_results = results.get('pipeline_stages', {}).get('contextual_ai_enhancement', {})
    if stage_results.get('status') == 'completed':
        metadata = stage_results.get('stage_metadata', {})
        print(f"Processing Time: {stage_results.get('processing_time_seconds', 0):.2f}s")
        ...
        if contextual_analysis:
            ...
            print(f"  Recommended Approach: {summary.get('recommended_approach', 'Standard')}")
