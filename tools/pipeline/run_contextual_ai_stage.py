...
async def run_contextual_ai_stage():
    ...
    try:
        ...
        stage_info = results.get('pipeline_stages', {}).get('contextual_ai_enhancement', {})
        ...
        if stage_info.get('status') == 'completed':
            ...
            if success:
                ...
                logger.info(f"  - Horses analyzed: {metadata.get('horses_analyzed', 0)}")
                logger.info(f"  - Alerts generated: {metadata.get('alerts_generated', 0)}")
                logger.info(f"  - Value opportunities: {metadata.get('value_opportunities', 0)}")
                logger.info(f"  - Confidence level: {metadata.get('confidence_level', 0):.2f}")
...
def create_sample_pipeline_data() -> Dict[str, Any]:
    ...
    return {
        ...
        'pipeline_metadata': {
            ...
            'previous_stages_completed': ['data_download', 'csv_import', 'data_quality', 'advanced_processing', 'ml_ensemble', 'performance_tracking', 'betting_integration']
        }
    }
    ...
    try:
        ...
        if contextual_analysis:
            ...
            with open(summary_path, 'w') as f:
                json.dump({
                    'race_id': results.get('race_data', {}).get('race_id', 'unknown'),
                    'timestamp': datetime.now().isoformat(),
                    'contextual_summary': contextual_analysis.get('contextual_summary', {}),
                    'alerts_count': len(contextual_analysis.get('race_conditions', {})),
                    'value_opportunities': len(contextual_analysis.get('value_assessment', {}).get('value_horses', [])),
                    'confidence_level': contextual_analysis.get('confidence_analysis', {}).get('overall_confidence', 0)
