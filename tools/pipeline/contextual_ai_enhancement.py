...
class ContextualAIEngine:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.market_movement_threshold = self.config.get(
    ...
    async def run_contextual_ai_analysis(
        ...
        try:
            ...
            cache_key = (
                f"{race_data.get('race_id')}_{datetime.now().strftime('%Y%m%d_%H')}"
            )
    ...
    async def _analyze_race_conditions(self, race_data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            weather_factors = self.weather_impact_factors.get(
                weather.lower(),
                {
                    "speed_impact": 0,
                    "stamina_bias": 0,
            ...
            track_factors = self.track_condition_factors.get(
                track_condition.lower(),
                {
                    "pace_bias": 0,
                    "distance_impact": 1.0,
            ...
            # Going analysis
            going_impact = self._analyze_going_impact(
                going, race_data.get("distance", 1600)
            )
            ...
            conditions["condition_impact_score"] = (
                self._calculate_condition_impact_score(
    ...
    async def _generate_form_insights(
        ...
        try:
            ...
            for horse in horses_data:
                ...
                individual_insight = await self._analyze_individual_form(
                    horse, race_data
                )
            ...
            insights["class_analysis"] = self._analyze_class_relationships(
                horses_data, race_data
            )

            # Recent form trends across the field
            insights["recent_form_trends"] = self._analyze_field_form_trends(
                horses_data
            )
    ...
    async def _analyze_market_sentiment(
        ...
        try:
            ...
            if not market_data:
                ...
                sentiment["market_moves"] = self._analyze_market_movements(
                    market_data, horses_data
                )
                sentiment["public_confidence"] = self._assess_public_confidence(
                    market_data
                )
                ...
                sentiment["sentiment_indicators"] = (
                    self._calculate_sentiment_indicators(market_data)
                )
                sentiment["market_efficiency"] = self._assess_market_efficiency(
                    market_data
                )
    ...
    async def _generate_intelligent_alerts(
        ...
        try:
            ...
            alerts = sorted(
                alerts, key=lambda x: (x.get("priority", 5), -x.get("confidence", 0))
            )
    ...
    async def _generate_race_preview(
        ...
        try:
            ...
            preview["race_summary"] = self._generate_race_summary(
                race_data, race_conditions
            )
            ...
            preview["ai_verdict"] = self._generate_ai_verdict(
                preview["key_contenders"],
                preview["value_opportunities"],
    ...
    async def _assess_contextual_value(
        ...
        try:
            ...
            for horse in horses_data:
                ...
                contextual_value = (
                    base_value * context_multiplier * sentiment_adjustment
                )
    ...
    async def _analyze_ai_confidence(
        ...
        try:
            ...
            # Pattern reliability
            confidence["pattern_reliability"] = pattern_analysis.get(
                "pattern_confidence", 0
            )

            # Condition predictability
            confidence["condition_predictability"] = (
                self._assess_condition_predictability(race_conditions)
            )
    ...
    def _generate_contextual_summary(
        ...
        try:
            ...
            if len(value_horses) > 2:
                insights.append(
                    f"Multiple value opportunities identified ({len(value_horses)} horses)"
                )
    ...
    def _initialize_weather_factors(self) -> Dict:
        ...
        return {
            ...
            "light_rain": {
                ...
                "stamina_bias": 0.1,
                "draw_bias": -0.1,
                "jockey_skill_importance": 1.1,
            },
            "heavy_rain": {
                "speed_impact": -0.15,
                "stamina_bias": 0.2,
                "draw_bias": -0.2,
                "jockey_skill_importance": 1.3,
            },
            "windy": {
                "speed_impact": -0.1,
                "stamina_bias": 0.05,
                "draw_bias": 0.15,
                "jockey_skill_importance": 1.2,
            },
        }
    ...
    def _initialize_track_factors(self) -> Dict:
        ...
        return {
            ...
            "good_to_soft": {
                "pace_bias": -0.05,
                "distance_impact": 1.05,
                "form_reliability": 0.95,
                "upset_probability": 0.2,
            },
            "soft": {
                "pace_bias": -0.1,
                "distance_impact": 1.1,
                "form_reliability": 0.9,
                "upset_probability": 0.25,
            },
            "heavy": {
                "pace_bias": -0.2,
                "distance_impact": 1.2,
                "form_reliability": 0.8,
    ...
    def _get_track_recommendation(self, track_condition: str) -> str:
        ...
        return recommendations.get(
            track_condition.lower(), "Assess individual track preferences"
        )
    ...
    def _analyze_going_impact(self, going: str, distance: int) -> Dict:
        ...
        return {
            ...
            + (0.1 if "soft" in going.lower() and distance > 1800 else 0),
    ...
    def _analyze_time_factors(self, race_data: Dict) -> Dict:
        ...
        return {
            "race_time": race_time,
            "time_of_day_factor": (
                1.1 if "15:" in race_time or "16:" in race_time else 1.0
            ),
            "seasonal_factor": 1.05 if 4 <= current_month <= 9 else 0.95,
            "time_recommendation": (
                "Prime racing conditions" if "15:" in race_time else "Standard timing"
            ),
        }
    ...
    def _calculate_condition_impact_score(
        ...
        weather_impact = abs(weather_factors.get("speed_impact", 0)) + abs(
            weather_factors.get("stamina_bias", 0)
        )
        track_impact = abs(track_factors.get("pace_bias", 0)) + (
            1 - track_factors.get("form_reliability", 1)
        )
    ...
    async def _analyze_individual_form(self, horse: Dict, race_data: Dict) -> Dict:
        ...
        return {
            ...
            "class_assessment": np.random.choice(
                ["drop_in_class", "suitable_level", "step_up"]
            ),
    ...
    def _analyze_class_relationships(
        self, horses_data: List[Dict], race_data: Dict
    ) -> Dict:
    ...
    def _identify_key_form_factors(
        self, horses_data: List[Dict], race_data: Dict
    ) -> List[str]:
    ...
    def _analyze_distance_patterns(
        self, horses_data: List[Dict], race_data: Dict
    ) -> List[Dict]:
    ...
    def _calculate_pattern_confidence(
        self,
        trainer_patterns: List,
        jockey_patterns: List,
        distance_patterns: List,
        emerging_patterns: List,
    ) -> float:
        """Calculate overall pattern confidence."""
        pattern_count = (
            len(trainer_patterns)
            + len(jockey_patterns)
            + len(distance_patterns)
            + len(emerging_patterns)
        )
        if pattern_count == 0:
            return 0.5
    ...
    def _generate_simulated_market_sentiment(self, horses_data: List[Dict]) -> Dict:
        ...
        return {
            "market_moves": (
                [
                    {
                        "horse": horses_data[0].get("horse_name", "Unknown"),
                        "movement": "backed",
    ...
    def _analyze_market_movements(
        self, market_data: Dict, horses_data: List[Dict]
    ) -> List[Dict]:
    ...
    def _identify_value_divergences(
        self, market_data: Dict, horses_data: List[Dict]
    ) -> List[Dict]:
    ...
    def _calculate_sentiment_indicators(self, market_data: Dict) -> Dict:
        ...
        return {"market_strength": 0.7, "liquidity": "good", "volatility": "low"}
    ...
    def _calculate_sentiment_adjustment(
        self, horse: Dict, market_sentiment: Dict
    ) -> float:
    ...
    def _calculate_value_confidence(
        ...
        condition_adjustment = (
            1.0 - race_conditions.get("condition_impact_score", 0) * 0.2
        )
    ...
    def _calculate_overall_confidence(
        ...
        condition_predictability: float,
        risk_factor_count: int,
    ) -> float:
        """Calculate overall confidence."""
        base_confidence = (
            data_quality + pattern_reliability + condition_predictability
        ) / 3
...
async def main():
    ...
    print(
        f"  Track Condition: {race_conditions.get('track_analysis', {}).get('track_condition', 'Unknown')}"
    )
    print(
        f"  Condition Impact Score: {race_conditions.get('condition_impact_score', 0):.2f}"
    )
    ...
    for i, alert in enumerate(alerts[:3], 1):  # Show top 3
        print(
            f"  {i}. {alert.get('type', 'Unknown')}: {alert.get('message', 'No message')}"
        )
    ...
    for horse in value_horses:
        print(
            f"  • {horse.get('horse_name', 'Unknown')}: {horse.get('contextual_value', 0):.2f} value"
        )
    ...
    print(f"\n🎯 Contextual Summary:")
    print(
        f"  Confidence Level: {contextual_summary.get('confidence_level', 'UNKNOWN')}"
    )
    print(f"  Race Character: {contextual_summary.get('race_character', 'Unknown')}")
    print(
        f"  Recommended Approach: {contextual_summary.get('recommended_approach', 'Standard analysis')}"
    )
