...
class DatabaseRelationshipTester:
    ...
    def analyze_potential_relationships(self):
        ...
        self.cursor.execute("SELECT COUNT(DISTINCT horse) FROM records WHERE horse IS NOT NULL")
        unique_horses_in_records = self.cursor.fetchone()[0]
        
        self.cursor.execute("SELECT COUNT(DISTINCT horse_name) FROM horses WHERE horse_name IS NOT NULL")
        ...
        if horses_missing_from_horses_table > 0:
            print(f"    ⚠️  {horses_missing_from_horses_table} horses in records not found in horses table")
        else:
        ...
        self.cursor.execute("SELECT COUNT(DISTINCT jockey) FROM records WHERE jockey IS NOT NULL")
        unique_jockeys_in_records = self.cursor.fetchone()[0]
        
        self.cursor.execute("SELECT COUNT(DISTINCT jockey_name) FROM jockeys_stats WHERE jockey_name IS NOT NULL")
        ...
        print(f"    Unique jockeys in jockeys_stats table: {unique_jockeys_in_jockeys_table}")
        
        if jockeys_missing_from_jockeys_table > 0:
            print(f"    ⚠️  {jockeys_missing_from_jockeys_table} jockeys in records not found in jockeys_stats table")
        else:
        ...
        self.cursor.execute("SELECT COUNT(DISTINCT trainer) FROM records WHERE trainer IS NOT NULL")
        unique_trainers_in_records = self.cursor.fetchone()[0]
        
        self.cursor.execute("SELECT COUNT(DISTINCT trainer_name) FROM trainers_stats WHERE trainer_name IS NOT NULL")
        ...
        print(f"    Unique trainers in trainers_stats table: {unique_trainers_in_trainers_table}")
        
        if trainers_missing_from_trainers_table > 0:
            print(f"    ⚠️  {trainers_missing_from_trainers_table} trainers in records not found in trainers_stats table")
        else:
    ...
    def check_data_quality(self):
        ...
        for table, columns in tables_to_check.items():
            ...
            for column in columns:
                # Check NULL values
                self.cursor.execute(f"SELECT COUNT(*) FROM {table} WHERE {column} IS NULL OR {column} = ''")
                null_count = self.cursor.fetchone()[0]
                
                percentage = (null_count / total_rows * 100) if total_rows > 0 else 0
                
                if null_count > 0:
                    print(f"    ⚠️  {column}: {null_count}/{total_rows} NULL/empty ({percentage:.1f}%)")
                else:
    ...
    def run_all_tests(self):
        ...
        print(f"🏇 Database Relationship Test - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("=" * 80)
