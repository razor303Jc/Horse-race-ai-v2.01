...
class IntelligentCachingLayer:
    ...
    def _initialize_invalidation_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            "race_update": ["race_data:*", "predictions:race:*", "analytics:race:*"],
            "model_update": ["predictions:*", "ml_data:*"],
            "data_import": ["analytics:*", "reports:*", "statistics:*"],
            "daily_rollover": ["daily_stats:*", "performance_metrics:*"],
        }
    ...
    async def initialize_database(self):
        ...
        try:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_invalidation_timestamp ON cache_invalidation_log(timestamp)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_performance_timestamp ON cache_performance_snapshots(timestamp)"
            )
    ...
    async def get(
        self, key: str, default: Any = None, strategy: Optional[str] = None
    ) -> Any:
        ...
        try:
            ...
            if value is not None:
                ...
                self.l1_cache.put(key, value)
                if self.l2_cache:
                    await self._put_to_redis(
                        key, value, cache_config.get("ttl", self.default_ttl)
                    )
    ...
    async def invalidate_by_tags(self, tags: List[str]) -> int:
        ...
        try:
            ...
            await self._log_cache_invalidation(
                "tag_invalidation", tags, invalidated_count
            )
    ...
    async def _put_to_database_cache(
        self, key: str, value: Any, ttl: int, tags: List[str]
    ):
    ...
    async def _remove_database_cache_pattern(self, pattern: str) -> int:
        ...
        try:
            ...
            matching_keys = [
                key for key in all_keys if self._match_pattern(key, pattern)
            ]

            if matching_keys:
                placeholders = ",".join(["?" for _ in matching_keys])
                cursor.execute(
                    f"DELETE FROM cache_l3_storage WHERE cache_key IN ({placeholders})",
                    matching_keys,
                )
        ...
        try:
            for query_config in self.warming_queries:
                try:
                    ...
                    # Log warming status
                    await self._log_cache_warming(
                        query_config["key"], warming_duration, success
                    )

                except Exception as e:
                    logger.warning(
                        f"Error warming cache for {query_config['key']}: {e}"
                    )
                    failed_count += 1
    ...
    async def _log_cache_invalidation(
        self, trigger: str, patterns: List[str], keys_invalidated: int
    ):
    ...
    async def _update_performance_metrics(self):
        ...
        try:
            ...
            total_hits = (
                self.performance_stats["l1_stats"]["hits"]
        ...
        try:
            ...
            return {
                ...
                "warming_summary": {
                    ...
                    "success_rate": round(
                        (warming_summary[1] or 0)
                        / max(warming_summary[0] or 1, 1)
                        * 100,
                        1,
                    ),
                ...
                "configuration": {
                    ...
                    "cache_warming_enabled": self.config.get(
                        "cache_warming_enabled", True
                    ),
...
async def main():
    ...
    try:
        ...
        await cache.put(
            "test_key",
            {"data": "test_value", "timestamp": datetime.now().isoformat()},
            strategy="race_data",
            tags=["test", "demo"],
        )
        ...
        await cache.put(
            "race_123", {"race_id": 123, "horses": [1, 2, 3]}, strategy="race_data"
        )
        await cache.put(
            "horse_456", {"horse_id": 456, "name": "Thunderbolt"}, strategy="horse_data"
        )
        await cache.put(
            "prediction_789",
            {"race_id": 123, "predictions": [0.7, 0.2, 0.1]},
        ...
        print(
            f"   - Invalidation events: {invalidation_summary['invalidation_events']}"
        )
        print(
            f"   - Keys invalidated: {invalidation_summary['total_keys_invalidated']}"
        )
