...
class DailyOperationsV205:
    ...
    def __init__(self, connection_config: Optional[Dict[str, str]] = None):
        ...
        self.processor = DataProcessorV205(
            connection_config=self.connection_config, batch_size=1000
        )
    ...
    def _execute_processing_phase(self, data_date: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate metrics
            total_processed = sum(
                stats.processed_records for stats in processing_results.values()
            )

            processing_success = all(
                stats.error_records == 0 for stats in processing_results.values()
            )
            ...
            return {
                ...
                "processing_errors": sum(
                    stats.error_records for stats in processing_results.values()
                ),
                "phase_time_seconds": phase_time,
                "entity_details": {
                    entity: {
    ...
    def _execute_upload_phase(self, data_date: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate metrics
            total_uploaded = sum(
                result.records_uploaded for result in upload_results.values()
            )

            upload_success = all(result.success for result in upload_results.values())
            ...
            return {
                ...
                "upload_failures": sum(
                    result.records_failed for result in upload_results.values()
                ),
                "phase_time_seconds": phase_time,
                "entity_details": {
                    entity: {
    ...
    def simulate_daily_operations(
        self, start_date: str, days: int = 1
    ) -> Dict[str, Any]:
...
def main():
    ...
    try:
        ...
        print(
            f"⏱️ Total Time: {pipeline_result['performance_metrics']['total_pipeline_time_seconds']:.2f}s"
        )
        ...
        for phase_name, phase_data in pipeline_result["phases"].items():
            ...
            print(
                f"{status} {phase_name}: {phase_data.get('records_processed', 0)} records"
            )
        ...
        if simulate_multiple == "y":
            ...
            print(
                f"📊 Success Rate: {simulation_result['summary_metrics']['success_rate_percentage']:.1f}%"
            )
            print(
                f"📈 Total Records: {simulation_result['summary_metrics']['total_records_processed']}"
            )
