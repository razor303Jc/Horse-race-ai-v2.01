...
class DataProcessorV205:
    ...
    def extract_csv_from_archive(self, archive_path: str,
    ...
    def process_entity_csv(self, csv_file: str, entity_type: str,
                          validate_data: bool = True) -> ProcessingStats:
    ...
    def _transform_entity_data(self, df: pd.DataFrame,
                ...
                for entity_col, csv_col in csv_mapping.items():
                    if csv_col in df.columns:
                        ...
                        if pd.isna(value):
                            ...
                            # Convert data types
                            record[entity_col] = self._convert_value(
                                value, entity_col
                            )
                    else:
                        record[entity_col] = None
    ...
    def _convert_value(self, value: Any, field_name: str) -> Any:
        ...
        if field_name.endswith('_id') or field_name in [
            'age', 'total_races', 'total_wins', 'total_placed',
            'flat_runners', 'flat_wins', 'jumps_runners', 'jumps_wins'
        ]:
            try:
    ...
    def _insert_entity_batch(self, records: List[Dict[str, Any]],
                           table_name: str) -> int:
    ...
    def process_complete_dataset(self, data_date: str) -> Dict[str, ProcessingStats]:
        ...
        if Path(cards_zip).exists():
            ...
            if horses_file:
                results['horses'] = self.process_entity_csv(
                    horses_file, 'horses'
                )
        ...
        try:
            with self.get_connection() as conn:
                ...
                verification_result = {
                    ...
                    "null_ids": null_ids,
                    "data_integrity": "PASS" if (
                        duplicate_count == 0 and null_ids == 0
                    ) else "FAIL"
                }
        ...
        for entity_type, stats in self.processing_stats.items():
            summary["entity_details"][entity_type] = {
                ...
                "records_per_second": (
                    ...
                    if stats.processing_time_seconds > 0 else 0
                )
            }
