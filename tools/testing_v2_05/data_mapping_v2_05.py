...
class DataMappingV205:
    ...
    def _initialize_entity_schemas(self) -> Dict[str, EntityTableSchema]:
        ...
        horses_schema = EntityTableSchema(
            ...
            field_mappings={
                "horse_id": FieldMapping("Horse_ID", "horse_id", FieldType.INTEGER, nullable=False),
                "horse_name": FieldMapping("Name", "horse_name", FieldType.VARCHAR, max_length=200, nullable=False),
                "country": FieldMapping("Country", "country", FieldType.VARCHAR, max_length=3),
                "age": FieldMapping("Age", "age", FieldType.INTEGER, validation_rules=["range:2,15"]),
                "colour": FieldMapping("colour", "colour", FieldType.VARCHAR, max_length=50),
                "sex": FieldMapping("sex", "sex", FieldType.VARCHAR, max_length=10),
                "owner": FieldMapping("owner", "owner", FieldType.TEXT),
                "trainer": FieldMapping("trainer", "trainer", FieldType.VARCHAR, max_length=200),
                "sire": FieldMapping("sire", "sire", FieldType.VARCHAR, max_length=200),
                "dam": FieldMapping("dam", "dam", FieldType.VARCHAR, max_length=200),
                "dam_sire": FieldMapping("dam_sire", "dam_sire", FieldType.VARCHAR, max_length=200)
            }
        )
        ...
        jockeys_schema = EntityTableSchema(
            ...
            field_mappings={
                "jockey_id": FieldMapping("jockey_ID", "jockey_id", FieldType.INTEGER, nullable=False),
                "jockey_name": FieldMapping("jockey", "jockey_name", FieldType.VARCHAR, max_length=200, nullable=False),
                "allowance_claimed": FieldMapping("allowance_claimed", "allowance_claimed", FieldType.BOOLEAN, default_value=False),
                "total_races": FieldMapping("races", "total_races", FieldType.INTEGER, validation_rules=["min:0"]),
                "total_wins": FieldMapping("wins", "total_wins", FieldType.INTEGER, validation_rules=["min:0"]),
                "win_percentage": FieldMapping("win_rate", "win_percentage", FieldType.DECIMAL, validation_rules=["range:0,100"]),
                "total_placed": FieldMapping("placed", "total_placed", FieldType.INTEGER, validation_rules=["min:0"]),
                "place_percentage": FieldMapping("place_rate", "place_percentage", FieldType.DECIMAL, validation_rules=["range:0,100"])
            }
        )
        ...
        trainers_schema = EntityTableSchema(
            ...
            indexes=["trainer_name", "win_percentage", "flat_win_rate", "jumps_win_rate"],
            field_mappings={
                "trainer_id": FieldMapping("trainer_ID", "trainer_id", FieldType.INTEGER, nullable=False),
                "trainer_name": FieldMapping("trainer", "trainer_name", FieldType.VARCHAR, max_length=200, nullable=False),
                "total_runners": FieldMapping("runners", "total_runners", FieldType.INTEGER, validation_rules=["min:0"]),
                "total_wins": FieldMapping("wins", "total_wins", FieldType.INTEGER, validation_rules=["min:0"]),
                "win_percentage": FieldMapping("win_rate", "win_percentage", FieldType.DECIMAL, validation_rules=["range:0,100"]),
                "total_placed": FieldMapping("placed", "total_placed", FieldType.INTEGER, validation_rules=["min:0"]),
                "place_percentage": FieldMapping("place_rate", "place_percentage", FieldType.DECIMAL, validation_rules=["range:0,100"]),
                "flat_runners": FieldMapping("flat_runners", "flat_runners", FieldType.INTEGER, validation_rules=["min:0"]),
                "flat_wins": FieldMapping("flat_wins", "flat_wins", FieldType.INTEGER, validation_rules=["min:0"]),
                "flat_win_rate": FieldMapping("flat_win_rate", "flat_win_rate", FieldType.DECIMAL, validation_rules=["range:0,100"]),
                "jumps_runners": FieldMapping("jumps_runners", "jumps_runners", FieldType.INTEGER, validation_rules=["min:0"]),
                "jumps_wins": FieldMapping("jumps_wins", "jumps_wins", FieldType.INTEGER, validation_rules=["min:0"]),
                "jumps_win_rate": FieldMapping("jumps_win_rate", "jumps_win_rate", FieldType.DECIMAL, validation_rules=["range:0,100"])
            }
        )
    ...
    def validate_csv_columns(self, csv_file: str, entity_type: str) -> Tuple[bool, List[str]]:
            ...
            if csv_col in csv_row.index and pd.notna(csv_row[csv_col]):
                ...
                try:
                    ...
                    if self._validate_field_value(converted_value, field_mapping):
                        ...
                        logger.warning(f"Validation failed for {entity_col}: {converted_value}")
    ...
    def _convert_data_type(self, value: Any, field_mapping: FieldMapping) -> Any:
        ...
        try:
            ...
            logger.warning(f"Data type conversion failed: {value} -> {field_mapping.field_type}: {str(e)}")
            return None
    ...
    def generate_insert_sql(self, entity_type: str, mapped_data: List[Dict[str, Any]]) -> Tuple[str, List[Tuple]]:
    ...
    def validate_entity_data_integrity(self, entity_type: str, mapped_data: List[Dict[str, Any]]) -> Tuple[bool, List[str]]:
            ...
            for constraint_col in schema.unique_constraints:
                ...
                if value is not None:
                    if value in unique_values:
                        errors.append(f"Duplicate {constraint_col} value at row {i}: {value}")
