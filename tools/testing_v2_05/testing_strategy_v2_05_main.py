...
def run_testing_only(data_date: str) -> bool:
    ...
    print_phase_header(
        "TESTING SUITE ONLY", 
        "Comprehensive testing without simulation scenarios"
    )
    ...
    try:
        ...
        for phase_name, phase_data in test_results["test_phases"].items():
            ...
            print(f"{status} {phase_name}: {phase_data.get('phase_time_seconds', 0):.2f}s")
...
def run_daily_pipeline(data_date: str) -> bool:
    ...
    print_phase_header(
        ...
        f"Single day processing pipeline for {data_date}"
    )
...
def main():
    ...
    parser.add_argument(
        "--date", 
        default="2025-08-26",
        help="Test data date (YYYY-MM-DD format)"
    )
