...
class DataValidationV205:
    ...
    def validate_csv_file(
        self, csv_file: str, entity_type: Optional[str] = None
    ) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                is_valid=False, errors=[f"CSV validation error: {str(e)}"]
            )
    ...
    def _validate_entity_data(
        ...
        for num_col in spec.get("numeric_columns", []):
            if num_col in df.columns:
                non_numeric = pd.to_numeric(df[num_col], errors="coerce").isna().sum()
                if non_numeric > 0:
                    result.warnings.append(
    ...
    def _validate_data_quality(self, df: pd.DataFrame) -> ValidationResult:
        ...
        if null_percentage > 50:
            result.warnings.append(f"High null percentage: {null_percentage:.1f}%")
        elif null_percentage > 20:
            result.warnings.append(f"Moderate null percentage: {null_percentage:.1f}%")
        ...
        if mixed_types:
            result.warnings.append(f"Mixed data types in columns: {mixed_types}")
    ...
    def validate_postgresql_schema(
        self, connection_config: Dict[str, str], entity_type: str
    ) -> ValidationResult:
        ...
        try:
            ...
            if missing_required:
                result.warnings.append(f"Missing expected columns: {missing_required}")
    ...
    def generate_data_quality_report(self, csv_files: List[str]) -> DataQualityReport:
        ...
        aggregate_metrics = {
            ...
            "validation_success_rate": sum(1 for r in all_results if r.is_valid)
    ...
    def validate_referential_integrity(
        self, connection_config: Dict[str, str], csv_files: Dict[str, str]
    ) -> ValidationResult:
        ...
        try:
            ...
            if "horses" in csv_files and "trainers" in csv_files:
                ...
                if (
                    "trainer_ID" in horses_df.columns
                    and "trainer_ID" in trainers_df.columns
                ):
            ...
            return ValidationResult(
                is_valid=False, errors=[f"Integrity check error: {str(e)}"]
            )

    def export_validation_report(
        self, report: DataQualityReport, output_file: str
    ) -> bool:
...
def main():
    ...
    if Path(cards_zip).exists():
        with zipfile.ZipFile(cards_zip, "r") as zip_ref:
            ...
            test_files.extend(
                [str(temp_dir / "horses/horses.csv"), str(temp_dir / "races/races.csv")]
            )
