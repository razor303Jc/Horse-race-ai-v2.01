...
class UploadSystemV205:
    ...
    def upload_entity_data(self, csv_file: str, entity_type: str) -> UploadResult:
        ...
        try:
            ...
            if not Path(csv_file).exists():
                return UploadResult(
                    success=False, error_message=f"CSV file not found: {csv_file}"
                )
            ...
            if not entity_config:
                return UploadResult(
                    success=False, error_message=f"Unknown entity type: {entity_type}"
                )
            ...
            if self.config.validate_before_upload:
                validation_errors = self._validate_data(transformed_data, entity_config)
            ...
            for batch in self._create_batches(transformed_data, self.config.batch_size):
            ...
            result = UploadResult(
                ...
                performance_metrics={
                    "records_per_second": (
                        total_uploaded / upload_time if upload_time > 0 else 0
                    ),
                    "average_batch_time": upload_time
                    / max(1, len(transformed_data) // self.config.batch_size),
            ...
            logger.info(
                f"✅ Upload completed: {total_uploaded} uploaded, {total_failed} failed"
            )
            return result
            ...
            return UploadResult(
                success=False, upload_time_seconds=upload_time, error_message=str(e)
            )
    ...
    def _transform_data(
        self, df: pd.DataFrame, entity_config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _validate_data(
        self, data: List[Dict[str, Any]], entity_config: Dict[str, Any]
    ) -> List[str]:
    ...
    def _upload_batch(
        self, batch: List[Dict[str, Any]], entity_config: Dict[str, Any]
    ) -> UploadResult:
        ...
        while attempt < self.config.retry_attempts:
            try:
                with self.get_connection() as conn:
                    ...
                    execute_values(
                        cursor, sql, data_tuples, template=f"({placeholders})"
                    )

                    conn.commit()

                    return UploadResult(success=True, records_uploaded=len(batch))
                ...
                if attempt < self.config.retry_attempts:
                    ...
                    return UploadResult(
                        success=False, records_failed=len(batch), error_message=str(e)
                    )
    ...
    def upload_complete_dataset(self, data_date: str) -> Dict[str, UploadResult]:
        ...
        processor = DataProcessorV205(
            connection_config=self.connection_config, batch_size=self.config.batch_size
        )
        ...
        try:
            with self.get_connection() as conn:
                ...
                verification_result = {
                    ...
                    "integrity_status": (
                        "PASS" if (null_uniques == 0 and duplicates == 0) else "FAIL"
                    ),
                }
    ...
    def get_upload_statistics(self) -> Dict[str, Any]:
        ...
        successful_uploads = sum(
            1 for entry in self.upload_history if entry["result"].success
        )

        total_records = sum(
            entry["result"].records_uploaded for entry in self.upload_history
        )

        total_time = sum(
            entry["result"].upload_time_seconds for entry in self.upload_history
        )
...
def main():
    ...
    uploader = UploadSystemV205(
        upload_config=UploadConfig(
            batch_size=500, max_workers=2, validate_before_upload=True
        )
    )
