...
class HistoricalMetricsGenerator:
    ...
    def __init__(self):
        self.results_data_dir = Path(
            "/home/jc/Documents/Horse-race-ai-v2.03/data/daily_downloads/results_data"
        )
    ...
    def generate_historical_metrics(self) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"   📈 {summary['metrics_generated']['power_ratings']} Power Ratings"
            )
            logger.info(
                f"   🏁 {summary['metrics_generated']['speed_ratings']} Speed Ratings"
            )
            logger.info(
                f"   📋 {summary['metrics_generated']['form_scores']} Form Scores"
            )
            logger.info(
                f"   🎲 {summary['metrics_generated']['monte_carlo']} Monte Carlo Records"
            )
    ...
    def _generate_metrics_from_results(self, results_data: Dict) -> Dict:
        ...
        for record in results_data["records"]:
            try:
                ...
                power_rating = self._calculate_power_rating_from_result(
                    record, race_info
                )
                if power_rating:
                ...
                speed_rating = self._calculate_speed_rating_from_result(
                    record, race_info
                )
                if speed_rating:
                ...
                # Generate form score
                form_score = self._calculate_form_score_from_result(
                    record, race_info, horse_info
                )
                if form_score:
                ...
                logger.warning(
                    f"⚠️ Error processing record {record.get('ID', 'unknown')}: {e}"
                )
                continue

        return metrics

    def _calculate_power_rating_from_result(
        self, record: Dict, race_info: Dict
    ) -> Optional[Dict]:
        ...
        try:
            ...
            return {
                ...
                "surface_type": (
                    "turf" if "Turf" in race_info.get("Race_type", "") else "aw"
                ),
    ...
    def _calculate_speed_rating_from_result(
        self, record: Dict, race_info: Dict
    ) -> Optional[Dict]:
    ...
    def _calculate_form_score_from_result(
        self, record: Dict, race_info: Dict, horse_info: Dict
    ) -> Optional[Dict]:
    ...
    def _calculate_monte_carlo_from_result(
        self, record: Dict, race_info: Dict
    ) -> Optional[Dict]:
            ...
            try:
                ...
                """,
                    (
                        ...
                        rating["power_rating"],
                        rating["base_rating"],
                        rating["class_adjustment"],
                        rating["race_class"],
                        rating["surface_type"],
                        rating["going_description"],
                        rating["distance_furlongs"],
                        rating["consistency_rating"],
                        rating["rating_confidence"],
                        rating["sample_size"],
            ...
            try:
                ...
                """,
                    (
                        ...
                        score["recent_form_trend"],
                        score["days_since_last_run"],
                        score["form_cycle"],
    ...
    def _save_json_files(self, metrics: Dict) -> Dict:
        ...
        historical_file = (
            self.output_dir
            / "historical_analysis"
            / f"historical_metrics_{timestamp}.json"
        )
        ...
        ml_file = (
            self.output_dir / "ml_training_data" / f"training_features_{timestamp}.json"
        )
        with open(ml_file, "w") as f:
            ...
            for i, rating in enumerate(metrics["power_ratings"]):
                try:
                    speed_rating = (
                        metrics["speed_ratings"][i]
                        if i < len(metrics["speed_ratings"])
                        else {}
                    )
                    form_score = (
                        metrics["form_scores"][i]
                        if i < len(metrics["form_scores"])
                        else {}
                    )
                    monte_carlo = (
                        metrics["monte_carlo"][i]
                        if i < len(metrics["monte_carlo"])
                        else {}
                    )
