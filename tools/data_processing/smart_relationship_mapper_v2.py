...
class SmartRelationshipMapper:
    """Advanced relationship mapper using name analysis."""
    ...
    def __init__(self):
        ...
        # Name mapping caches
        self.jockey_mappings: Dict[str, str] = {}
    ...
    def load_reference_names(self):
        ...
        logger.info(
            f"Loaded {len(self.jockey_stats_names)} jockey names, {len(self.trainer_stats_names)} trainer names"
        )

    def find_best_name_match(
        self, target_name: str, candidates: List[str], threshold: float = 0.85
    ) -> Optional[str]:
    ...
    def extract_names_from_horses_table(self) -> Tuple[Dict[str, int], Dict[str, int]]:
        ...
        logger.info(
            f"Found {len(jockey_freq)} unique jockeys, {len(trainer_freq)} unique trainers in horses table"
        )
    ...
    def build_name_mappings(self):
        ...
        for horse_jockey, freq in jockey_freq.items():
            best_match = self.find_best_name_match(
                horse_jockey, self.jockey_stats_names
            )
            if best_match:
                self.jockey_mappings[horse_jockey] = best_match
                mapped_jockeys += 1
                logger.debug(
                    f"Jockey mapping: '{horse_jockey}' -> '{best_match}' (freq: {freq})"
                )
        ...
        for horse_trainer, freq in trainer_freq.items():
            best_match = self.find_best_name_match(
                horse_trainer, self.trainer_stats_names
            )
            if best_match:
                self.trainer_mappings[horse_trainer] = best_match
                mapped_trainers += 1
                logger.debug(
                    f"Trainer mapping: '{horse_trainer}' -> '{best_match}' (freq: {freq})"
                )

        logger.info(
            f"Built {mapped_jockeys} jockey mappings, {mapped_trainers} trainer mappings"
        )
    ...
    def update_race_results_via_horses(self) -> Tuple[int, int]:
        ...
        for horse_jockey, stats_jockey in self.jockey_mappings.items():
            ...
            """
            self.cursor.execute(update_query, (stats_jockey, horse_jockey))

            updated_rows = self.cursor.rowcount
            if updated_rows > 0:
                jockey_updates += updated_rows
                logger.debug(
                    f"Updated {updated_rows} races: jockey '{horse_jockey}' -> '{stats_jockey}'"
                )
        ...
        for horse_trainer, stats_trainer in self.trainer_mappings.items():
            ...
            """
            self.cursor.execute(update_query, (stats_trainer, horse_trainer))

            updated_rows = self.cursor.rowcount
            if updated_rows > 0:
                trainer_updates += updated_rows
                logger.debug(
                    f"Updated {updated_rows} races: trainer '{horse_trainer}' -> '{stats_trainer}'"
                )

        self.connection.commit()
        logger.info(
            f"Updated {jockey_updates} jockey entries, {trainer_updates} trainer entries"
        )
    ...
    def run_smart_mapping(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"   - Jockey mappings created: {final_results['mapped_jockeys']}"
            )
            logger.info(
                f"   - Trainer mappings created: {final_results['mapped_trainers']}"
            )
...
def main():
    ...
    print("Smart Horse Racing Database - Relationship Mapper")
