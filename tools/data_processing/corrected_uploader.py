...
def main():
    ...
    with conn.cursor() as cur:
        ...
        result = cur.fetchone()
        print(
            f"   records: {result[0]} total, {result[1]} with race_dates ({result[2]} to {result[3]})"
        )

        # Show sample race results
        cur.execute("SELECT race_id, horse_name, jockey_name FROM records LIMIT 3")
        ...
        result = cur.fetchone()
        print(
            f"   races: {result[0]} total, {result[1]} with dates ({result[2]} to {result[3]})"
        )

        # Show sample race cards
        cur.execute("SELECT race_number, course, date FROM races LIMIT 3")
        samples = cur.fetchall()
        print(f"   Sample races: {samples}")
