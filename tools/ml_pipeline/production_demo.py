...
def engineer_features_for_prediction(df, encoders):
    ...
    if "course" in encoders:
        try:
            df_features["course_encoded"] = encoders["course"].transform(
                df_features["course"].astype(str)
            )
        except ValueError:
...
def make_prediction_demo():
    ...
    print(
        f"   Model Confidence: {'High' if win_probability > 0.7 or win_probability < 0.3 else 'Medium'}"
    )
