...
class MLFeaturePipeline:
    ...
    def __init__(self):
        ...
        self.numeric_features = [
            ...
            'win_odds', 'place_odds', 'distance', 'prize_money'
        ]
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        race_stats.columns = ['race_id', 'field_size', 'min_odds', 'max_odds', 'avg_odds', 'race_prize']
        ...
        df['age_category'] = pd.cut(df['horse_age'], bins=[0, 3, 5, 7, 15], labels=[0, 1, 2, 3])
        ...
        self.engineered_features = [
            'log_odds', 'implied_probability', 'odds_rank', 'is_favorite',
            'combined_performance', 'field_size', 'min_odds', 'max_odds', 'avg_odds',
            'draw_percentile', 'weight_percentile', 'age_category'
        ]
        
        logger.info(f"✅ Engineered {len(self.engineered_features)} additional features")
        return df
    ...
    def create_preprocessing_pipeline(self) -> ColumnTransformer:
        ...
        all_numeric_features = [f for f in all_numeric_features if f not in ['age_category']]
        ...
        logger.info(f"✅ Created preprocessing pipeline for {len(self.feature_names)} features")
        return preprocessor
    
    def prepare_ml_dataset(self, test_size: float = 0.2) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """
        ...
        logger.info(f"   Training: {X_train_scaled.shape[0]} samples, {X_train_scaled.shape[1]} features")
        logger.info(f"   Testing: {X_test_scaled.shape[0]} samples, {X_test_scaled.shape[1]} features")
    ...
    def transform_new_data(self, df: pd.DataFrame) -> np.ndarray:
        ...
        """
        if self.preprocessor is None:
            raise ValueError("Preprocessing pipeline not fitted. Run prepare_ml_dataset first.")
        
        # Engineer features on new data
        df_engineered = self.engineer_features(df.copy())
        
        # Select features used in training
        feature_columns = [col for col in self.feature_names if col in df_engineered.columns]
        X_new = df_engineered[feature_columns]
        
        # Transform using fitted pipeline
        X_scaled = self.preprocessor.transform(X_new)
        
        logger.info(f"✅ Transformed {X_scaled.shape[0]} new samples with {X_scaled.shape[1]} features")
        return X_scaled
