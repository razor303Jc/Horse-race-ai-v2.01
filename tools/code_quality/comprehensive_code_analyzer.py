        ...
        try:
            ...
            return {
                ...
                'halstead_metrics': {
                    ...
                    'difficulty': sum(h.difficulty for h in halstead) if halstead else 0,
...
class SecurityAnalyzer:
    ...
    def _get_security_recommendation(self, test_id: str) -> str:
        ...
        return recommendations.get(test_id, "Review security implications and follow best practices")
            ...
            for error in report.get_statistics('E'):
                ...
                if len(parts) >= 4:
                    violation = StyleViolation(
                        ...
                        suggestion=self._get_style_suggestion(parts[3].strip().split()[0])
                    )
...
class DuplicationDetector:
    ...
    def analyze_project(self, project_path: str) -> Dict[str, Any]:
        ...
        try:
                        ...
                        if isinstance(node, (ast.FunctionDef, ast.ClassDef)):
                            ...
                            if code_block:
                                ...
                                if block_hash in file_hashes:
                                    duplications.append({
                                        'original_file': file_hashes[block_hash]['file'],
                                        'original_line': file_hashes[block_hash]['line'],
            ...
            duplication_percentage = (duplicate_blocks / total_blocks * 100) if total_blocks > 0 else 0
...
class DocumentationAnalyzer:
    ...
    def analyze_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            function_doc_score = (documented_functions / total_functions * 100) if total_functions > 0 else 100
            class_doc_score = (documented_classes / total_classes * 100) if total_classes > 0 else 100
            ...
            return {
                ...
                'documentation_issues': self._identify_documentation_issues(tree, content)
            }
    ...
    def _identify_documentation_issues(self, tree: ast.AST, content: str) -> List[Dict[str, Any]]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                if not docstring:
                    ...
                    if any(isinstance(n, ast.Return) and n.value for n in ast.walk(node)):
...
class CodeSmellDetector:
    ...
    def _detect_duplicate_code_in_file(self, file_path: str, content: str) -> List[CodeSmell]:
...
class ComprehensiveCodeQualityAnalyzer:
    ...
    def discover_python_files(self) -> List[str]:
        ...
        for root, dirs, files in os.walk(self.project_root):
            # Skip common directories
            dirs[:] = [d for d in dirs if d not in ['__pycache__', '.git', 'node_modules', 'venv', '.venv', 'build', 'dist']]
    ...
    def analyze_project(self) -> Dict[str, Any]:
        ...
        for file_path in python_files:
            try:
                ...
                file_metrics = CodeQualityMetrics(
                    ...
                    complexity=complexity.get('cyclomatic_complexity', {}).get('total', 0),
                    ...
                    documentation_score=documentation.get('overall_documentation_score', 0),
                ...
                results['security_issues'].extend([issue.__dict__ for issue in security_issues])
                results['style_violations'].extend([violation.__dict__ for violation in style_violations])
        ...
        duplication_analysis = self.duplication_detector.analyze_project(str(self.project_root))
        ...
        avg_doc_score = sum(fm['documentation_score'] for fm in results['file_metrics']) / len(python_files) if python_files else 0
        avg_overall_score = sum(fm['overall_score'] for fm in results['file_metrics']) / len(python_files) if python_files else 0
        ...
        results['project_metrics'] = {
            ...
            'duplication_percentage': duplication_analysis.get('duplication_percentage', 0),
...
def main():
    ...
    print(f"  Average Score: {results['project_metrics']['average_overall_score']:.1f}/100")
