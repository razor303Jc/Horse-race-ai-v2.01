...
class AdvancedMetricsPipeline:
    ...
    def _prepare_ml_training_data(self) -> dict:
        ...
        try:
            ...
            logger.info(f"      📊 Historical records: {consolidated_data['historical_records']}")
            logger.info(f"      📊 Current records: {consolidated_data['current_records']}")
    ...
    def _validate_database(self) -> dict:
        ...
        try:
            ...
            tables = ['horse_power_ratings', 'horse_speed_ratings', 'horse_form_scores', 'monte_carlo_simulations']
            table_stats = {}
            ...
            validation_result = {
                ...
                'data_quality': {
                    ...
                    'avg_power_rating': float(power_stats[1]) if power_stats and power_stats[1] else 0,
                    'power_rating_range': [float(power_stats[2]), float(power_stats[3])] if power_stats else [0, 0]
                }
            }
            
            logger.info(f"   ✅ Database validation complete")
            logger.info(f"      🗄️ Power ratings: {table_stats.get('horse_power_ratings', 0)} records")
            logger.info(f"      🗄️ Speed ratings: {table_stats.get('horse_speed_ratings', 0)} records")
            logger.info(f"      🗄️ Form scores: {table_stats.get('horse_form_scores', 0)} records")
            logger.info(f"      🗄️ Monte Carlo: {table_stats.get('monte_carlo_simulations', 0)} records")
        ...
        try:
            ...
            pipeline_config = {
                ...
                'ml_training': {
                    ...
                    'feature_selection': [
                        'power_rating', 'speed_figure', 'pace_rating', 'form_score',
                        'win_probability', 'age', 'weight', 'odds'
                    ]
                },
                'automation': {
            ...
            # Save configuration
            config_file = self.project_root / "config" / "advanced_metrics_pipeline.json"
            ...
            return {
                ...
                'features_enabled': len(pipeline_config['ml_training']['feature_selection'])
            }
