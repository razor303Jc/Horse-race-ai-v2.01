...
class DatabaseOptimizationAnalyzer:
    ...
    def analyze_query_performance(self) -> Dict[str, float]:
        ...
        test_queries = [
            ("select_by_date", "SELECT COUNT(*) FROM race_results WHERE race_date >= '2023-01-01'"),
            ("select_by_course", "SELECT COUNT(*) FROM race_results WHERE course = 'Flemington'"),
            ("select_by_jockey", "SELECT COUNT(*) FROM race_results WHERE jockey_name = 'Unknown'"),
            ("select_by_trainer", "SELECT COUNT(*) FROM race_results WHERE trainer_name = 'Unknown'"),
            ("join_horses_results", """
                SELECT COUNT(*) 
    ...
    def analyze_foreign_key_opportunities(self) -> List[Dict]:
        ...
        for relationship in relationship_queries:
            try:
                ...
                logger.warning(f"❌ Relationship analysis {relationship['name']} failed: {e}")
    ...
    def print_analysis_summary(self):
        ...
        if 'table_sizes' in self.analysis_results:
            ...
            for table, data in self.analysis_results['table_sizes'].items():
                if isinstance(data, dict) and 'records' in data:
                    print(f"   {table:20}: {data['records']:,} records ({data['size_pretty']})")
        ...
        if 'recommendations' in self.analysis_results:
            ...
            if len(self.analysis_results['recommendations']) > 5:
                print(f"      ... and {len(self.analysis_results['recommendations']) - 5} more")
