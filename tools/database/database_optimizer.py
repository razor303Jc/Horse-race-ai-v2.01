...
class DatabaseOptimizer:
    ...
    def create_performance_indexes(self) -> bool:
        ...
        for index in performance_indexes:
            try:
                ...
                logger.info(f"✅ Created index {index['name']} ({end_time - start_time:.2f}s)")
    ...
    def add_foreign_key_constraints(self) -> bool:
        ...
        for fk in foreign_keys:
            try:
                ...
                logger.info(f"✅ Created foreign key {fk['name']} ({end_time - start_time:.2f}s)")
                self.optimizations_applied.append({
                    'type': 'foreign_key',
                    'name': fk['name'],
    ...
    def add_check_constraints(self) -> bool:
        ...
        for constraint in check_constraints:
            try:
                ...
                if self.cursor.fetchone():
                    logger.info(f"⏭️  Check constraint {constraint['name']} already exists")
                    continue
                ...
                logger.info(f"✅ Created check constraint {constraint['name']} ({end_time - start_time:.2f}s)")
                self.optimizations_applied.append({
                ...
                logger.warning(f"⚠️  Could not create check constraint {constraint['name']}: {e}")
                self.connection.rollback()
    ...
    def optimize_storage(self) -> bool:
        ...
        tables = ['race_results', 'horses', 'jockey_stats', 'trainer_stats', 'races_cards', 'racecard_details']
    ...
    def test_performance_improvements(self) -> Dict[str, float]:
        ...
        test_queries = [
            ("date_course_query", "SELECT COUNT(*) FROM race_results WHERE race_date >= '2023-01-01' AND course = 'Flemington'"),
            ("jockey_performance", "SELECT jockey_name, COUNT(*), AVG(win_odds) FROM race_results WHERE jockey_name != 'Unknown' GROUP BY jockey_name LIMIT 10"),
            ("trainer_analysis", "SELECT trainer_name, COUNT(*) FROM race_results WHERE trainer_name != 'Unknown' GROUP BY trainer_name ORDER BY COUNT(*) DESC LIMIT 10"),
            ("winner_analysis", "SELECT course, COUNT(*) FROM race_results WHERE finished_position = 1 GROUP BY course ORDER BY COUNT(*) DESC LIMIT 10"),
            ("complex_join", """
        ...
        for test_name, query in test_queries:
            try:
                ...
                status = "🟢" if execution_time < 50 else "🟡" if execution_time < 100 else "🔴"
    ...
    def print_optimization_summary(self, results: Dict):
        ...
        print(f"   {'✅' if steps['storage_optimization'] else '❌'} Storage optimization")
