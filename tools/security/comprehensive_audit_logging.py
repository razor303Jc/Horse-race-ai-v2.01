...
class SQLiteAuditStorage(AuditStorage):
    ...
    def _row_to_event(self, row: sqlite3.Row) -> AuditEvent:
        ...
        return AuditEvent(
            ...
            compliance_tags=(
                json.loads(row["compliance_tags"]) if row["compliance_tags"] else []
            ),
                ...
                for field in requirement.required_fields:
                    ...
                    if field_value is None or (
                        ...
                        compliance_issues.append(
                            f"{req_name}: Missing required field '{field}'"
                        )
...
class ComprehensiveAuditLogger:
    ...
    def __init__(self, storage_backend: AuditStorage, async_processing: bool = True):
        ...
        if async_processing:
            ...
            self.processing_thread = threading.Thread(
                target=self._process_events_async, daemon=True
            )
    ...
    def log_event(
        ...
        if compliance_issues:
            self.logger.warning(
                f"Compliance issues for event {event.event_id}: {compliance_issues}"
            )
    ...
    def log_prediction_generated(
        self, user_id: str, race_id: str, prediction_details: Dict[str, Any]
    ):
    ...
    def generate_compliance_report(
        ...
        relevant_events = [
            e for e in all_events if e.event_type in requirement.event_types
        ]
...
def main():
    ...
    test_events = [
        (
            "User Login",
            lambda: audit_logger.log_user_login(
                "test_user", "192.168.1.100", "Mozilla/5.0 Browser"
            ),
        ),
        (
            "Data Access",
            lambda: audit_logger.log_data_access(
                "test_user", "horses_table", "SELECT * FROM horses"
            ),
        ),
        (
            "Prediction Generated",
            lambda: audit_logger.log_prediction_generated(
                "test_user",
                "20241201_NEW_1",
                {"horse": "Thunder Bolt", "confidence": 0.85},
            ),
        ),
        (
            "Bet Placed",
            lambda: audit_logger.log_bet_placed(
                "test_user",
                {"race_id": "20241201_NEW_1", "horse": "Thunder Bolt", "stake": 10.0},
            ),
        ),
        (
            "Security Alert",
            lambda: audit_logger.log_security_alert(
                "failed_login_attempts", {"attempts": 5, "ip": "192.168.1.100"}
            ),
        ),
        (
            "API Request",
            lambda: audit_logger.log_api_request(
    ...
    for event_name, event_func in test_events:
        success = event_func()
        print(
            f"  {'✅' if success else '❌'} {event_name}: {'Logged' if success else 'Failed'}"
        )
