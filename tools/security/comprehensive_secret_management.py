...
class ComprehensiveSecretManager:
    ...
    def store_secret(self, name: str, value: str, secret_type: str, 
        ...
        try:
            ...
            if success:
                ...
                # Log audit event
                self._audit_log("SECRET_STORED", {"secret_name": name, "secret_type": secret_type})
...
def main():
    ...
    secret_manager = ComprehensiveSecretManager(
        ...
        storage_config={"path": "config/secrets"}
    )
    ...
    test_secrets = [
        ("betdaq_api_key", "sample_betdaq_key_12345", "api_key", ["betting", "production"]),
        ("database_password", "secure_db_password_67890", "password", ["database", "production"]),
    ...
    for name, value, secret_type, tags in test_secrets:
        ...
        print(f"  {'✅' if success else '❌'} Store {name}: {'Success' if success else 'Failed'}")
    ...
    for secret_name in secret_manager.list_secrets():
        ...
        if metadata:
            print(f"  🔑 {secret_name} ({metadata.secret_type}) - {len(metadata.tags)} tags")
