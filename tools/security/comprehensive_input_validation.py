...
class SecurityValidator:
    ...
    def check_command_injection(self, value: str) -> List[str]:
        ...
        for pattern in self.command_injection_patterns:
            if re.search(pattern, value, re.IGNORECASE):
                security_flags.append(f"Potential command injection detected: {pattern}")
        
        return security_flags
    
    def validate_content_length(self, content: str, max_length: int = 1000000) -> List[str]:
        """Validate content length to prevent DoS"""
        security_flags = []
        
        if len(content) > max_length:
            security_flags.append(f"Content exceeds maximum length: {len(content)} > {max_length}")
...
class ComprehensiveInputValidator:
    ...
    def validate_data(self, data: Dict[str, Any], 
                     rules: Dict[str, ValidationRule]) -> ValidationResult:
        ...
        for field_name, rule in rules.items():
            ...
            if isinstance(value, str):
                security_flags.extend(self.security_validator.check_sql_injection(value))
                security_flags.extend(self.security_validator.check_xss(value))
                security_flags.extend(self.security_validator.check_command_injection(value))
                security_flags.extend(self.security_validator.validate_content_length(value))
            
            # Sanitize value
            sanitized_value = self._sanitize_value(value, rule)
            
            if sanitized_value is None and rule.required:
                field_errors.append(f"Invalid {rule.data_type} value for '{field_name}'")
                continue
    ...
    def _validate_value(self, value: Any, rule: ValidationRule) -> List[str]:
        ...
        if rule.data_type in ['integer', 'float', 'decimal'] and isinstance(value, (int, float, Decimal)):
    ...
    def validate_horse_data(self, horse_data: Dict[str, Any]) -> ValidationResult:
        ...
        rules = {
            ...
            'weight': ValidationRule(
                ...
                min_value=45.0, max_value=80.0
            ),
            'jockey': ValidationRule(
                'jockey', 'string', required=True,
                min_length=2, max_length=50
            ),
            'trainer': ValidationRule(
                'trainer', 'string', required=True,
                min_length=2, max_length=50
            ),
            'odds': ValidationRule(
                'odds', 'float', required=False,
