...
class RoleBasedAccessControl:
    ...
    def can_access_resource(self, user_roles: List[str], resource: str, action: str) -> bool:
...
class UserManager:
        ...
        try:
            ...
            with sqlite3.connect(self.database_path) as conn:
                ...
                ''', (
                    ...
                    user.salt, ','.join(user.roles), ','.join(user.permissions),
                    user.created_at.isoformat(), user.updated_at.isoformat(),
                    None, user.login_attempts, int(user.account_locked),
                    int(user.mfa_enabled), user.mfa_secret, user.session_timeout,
                    user.api_key, None
                ))
                conn.commit()
    ...
    def authenticate_user(self, username: str, password: str) -> Optional[User]:
        ...
        try:
            ...
            if user.account_locked:
                self.logger.warning(f"Authentication attempt on locked account: {username}")
                return None
    ...
    def create_session(self, user: User, ip_address: str, user_agent: str) -> Optional[Session]:
        """Create user session"""
        try:
            session = Session(
    ...
    def _row_to_user(self, row: sqlite3.Row) -> User:
        ...
        return User(
            ...
            api_key=row['api_key'],
            api_key_expires=datetime.fromisoformat(row['api_key_expires']) if row['api_key_expires'] else None
        )
...
class ComprehensiveAuthSystem:
    ...
    def register_user(self, username: str, email: str, password: str, 
                     roles: List[str] = None) -> AuthenticationResult:
        """Register new user"""
        try:
...
def main():
    ...
    for username, email, password, roles in test_users:
        ...
        print(f"  {'✅' if result.success else '❌'} Register {username}: "
              f"{'Success' if result.success else result.error_message}")
    
    print("\n🔑 Testing User Authentication:")
    
    # Test authentication
    login_result = auth_system.login("admin", "SecurePassword123!", "127.0.0.1", "Test Browser")
    print(f"  {'✅' if login_result.success else '❌'} Admin Login: "
