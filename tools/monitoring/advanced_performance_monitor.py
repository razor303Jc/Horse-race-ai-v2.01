...
class AlertManager:
    ...
    def check_resource_alerts(self, snapshot: ResourceSnapshot) -> List[PerformanceAlert]:
...
class AdvancedPerformanceMonitor:
    ...
    def __init__(self, monitoring_interval: int = 30):
        ...
        self.stage_timings: Dict[str, deque] = defaultdict(
    ...
    def collect_system_metrics(self) -> ResourceSnapshot:
        ...
        try:
            ...
            return ResourceSnapshot(
                ...
                disk_free_gb=0, process_count=0
            )

    def collect_pipeline_metrics(self, stage: str, processing_time: float,
                                 records_processed: int,
                                 errors: int = 0) -> PerformanceMetric:
        ...
        if (not baseline or
            (processing_time < baseline.processing_time * 0.9 and
             throughput > baseline.throughput * 1.1)):
            self.stage_baselines[stage] = metric

        return metric

    def analyze_performance_trends(self, stage: str,
    ...
    def create_monitoring_display(self) -> Layout:
        ...
        if latest_snapshot:
            ...
            cpu_status = ("🟢 Normal" if latest_snapshot.cpu_percent < 70
                         else "🟡 High" if latest_snapshot.cpu_percent < 90
                         else "🔴 Critical")
            system_table.add_row("CPU Usage",
                                 f"{latest_snapshot.cpu_percent:.1f}%",
                                 cpu_status)

            mem_status = ("🟢 Normal" if latest_snapshot.memory_percent < 70
                         else "🟡 High" if latest_snapshot.memory_percent < 85
                         else "🔴 Critical")
            ...
            system_table.add_row("Disk Free",
                                 f"{latest_snapshot.disk_free_gb:.1f} GB",
                                 "🟢 Normal")
            system_table.add_row("Processes",
                                 str(latest_snapshot.process_count),
                                 "🟢 Normal")
        else:
            system_table = Table(title="🖥️ System Performance")
        ...
        for stage, timings in list(self.stage_timings.items())[-5:]:
            if timings:
                ...
                if len(timings) >= 5:
                    ...
                    older_avg = (statistics.mean(list(timings)[-6:-3])
                                if len(timings) >= 6 else avg_time)
                    trend = ("📈 Slower" if recent_avg > older_avg * 1.1
                            else "📉 Faster" if recent_avg < older_avg * 0.9
                            else "➡️ Stable")
                else:
                    trend = "📊 Tracking"

                perf_table.add_row(stage, f"{avg_time:.2f}s",
                                  str(sample_count), trend)

        # Recent alerts panel
        alerts_text = Text()
        if recent_alerts:
            for alert in recent_alerts:
                severity_color = ("red" if alert.severity == "CRITICAL"
                                 else "yellow")
                alerts_text.append(f"[{alert.timestamp.strftime('%H:%M')}] ",
                                  style="dim")
                alerts_text.append(f"{alert.alert_type}: ",
                                  style=severity_color)
                alerts_text.append(f"{alert.message}\n", style="white")
        else:
            alerts_text.append("No recent alerts", style="green")

        alerts_panel = Panel(alerts_text, title="🚨 Recent Alerts",
...
"""
        monitoring_panel = Panel(monitoring_info, title="📈 Monitor Status",
                                border_style="blue")
    ...
    def run_continuous_monitoring(self):
        ...
        console.print("🚀 [bold green]Starting Advanced Performance Monitor[/bold green]")
        console.print(f"📊 Monitoring interval: {self.monitoring_interval} seconds")
        console.print("🔄 Press Ctrl+C to stop\n")

        try:
            with Live(self.create_monitoring_display(),
                     refresh_per_second=0.5) as live:
                while True:
            ...
            # Save report to file
            report_path = (self.project_root / "logs" / "monitoring" /
