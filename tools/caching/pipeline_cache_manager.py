...
class PipelineCacheManager:
    ...
    def get(
        self, cache_type: str, key_args: tuple, default: Any = None
    ) -> Optional[Any]:
        """
        Get data from cache.
    ...
    def set(
        self, cache_type: str, key_args: tuple, data: Any, ttl: Optional[int] = None
    ) -> bool:
        """
        Store data in cache.
        ...
        try:
            if self.redis_client:
                ...
                if success:
                    logger.debug(
                        f"💾 Cache SET: {cache_type} - {cache_key[:20]}... (TTL: {ttl}s)"
                    )
                    return True
                else:
                ...
                logger.debug(
                    f"💾 Fallback cache SET: {cache_type} - {cache_key[:20]}..."
                )
                return True
        ...
        try:
            if self.redis_client:
                ...
                if keys:
                    ...
                    logger.info(
                        f"🗑️  Invalidated {deleted} cache entries for {cache_type}"
                    )
                    return deleted
                return 0
            else:
                ...
                keys_to_delete = [
                    k for k in self._fallback_cache.keys() if k.startswith(prefix)
                ]
                ...
                logger.info(
                    f"🗑️  Invalidated {len(keys_to_delete)} fallback cache entries"
                )
...
def cached_feature_engineering(cache_manager: PipelineCacheManager, ttl: int = 3600):
...
if __name__ == "__main__":
    ...
    success = cache_manager.set(
        "features", ("R123456", "basic_features"), test_features
    )
