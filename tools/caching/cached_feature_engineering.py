...
class CachedFeatureEngineering:
    ...
    def __init__(
        ...
        self.cache_manager = PipelineCacheManager(
            redis_host=redis_host, redis_port=redis_port, default_ttl=cache_ttl
        )
    ...
    @cached_database_query
    def get_horse_historical_performance(
        self, horse_name: str, conn, days_back: int = 365
    ) -> pd.DataFrame:
        """
        ...
        try:
            df = pd.read_sql_query(
                query, conn, params=(horse_name, cutoff_date.strftime("%Y-%m-%d"))
            )
    ...
    def get_jockey_trainer_stats(
        self, jockey: str, trainer: str, conn, days_back: int = 365
    ) -> Dict[str, float]:
        """
        ...
        try:
            ...
            jockey_df = pd.read_sql_query(
                jockey_query, conn, params=(jockey, cutoff_date.strftime("%Y-%m-%d"))
            )

            # Get trainer stats
            trainer_df = pd.read_sql_query(
                trainer_query, conn, params=(trainer, cutoff_date.strftime("%Y-%m-%d"))
            )
            ...
            stats = {
                ...
                "jockey_win_rate": (
                    jockey_stats.get("wins", 0)
                    / max(jockey_stats.get("total_races", 1), 1)
                )
                * 100,
                "jockey_place_rate": (
                    jockey_stats.get("places", 0)
                    / max(jockey_stats.get("total_races", 1), 1)
                )
                * 100,
                ...
                "trainer_win_rate": (
                    trainer_stats.get("wins", 0)
                    / max(trainer_stats.get("total_races", 1), 1)
                )
                * 100,
                "trainer_place_rate": (
                    trainer_stats.get("places", 0)
                    / max(trainer_stats.get("total_races", 1), 1)
                )
                * 100,
            }
    ...
    @cached_feature_engineering
    def calculate_horse_form_features(
        self, horse_name: str, historical_data: pd.DataFrame
    ) -> Dict[str, float]:
        """
        ...
        try:
            ...
            recent_form = sum(6 - pos for pos in recent_positions if pos <= 5) / len(
                recent_positions
            )
        ...
        try:
            with self.get_database_connection() as conn:
                ...
                for _, horse_record in race_data.iterrows():
                    ...
                    jt_stats = self.get_jockey_trainer_stats(jockey, trainer, conn)
                ...
                logger.info(
                    f"✅ Created ML features for {len(features_df)} horses in {execution_time:.2f}s"
                )
