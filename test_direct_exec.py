...
def test_exec_node_command():
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={
                **os.environ,
                'PYTHONUNBUFFERED': '1'  # Same env var as exec node
            }
        )
...
def test_full_pipeline_command():
    ...
    full_command = [
        'docker', 'exec', 'horse_racing_data_pipeline_clean',
        'python', '/app/tools/manual_pipeline_trigger.py', '--all'
    ]
    ...
    try:
        ...
        exec_result = {
            ...
            "execution_time_seconds": round(execution_time, 2),
            "exit_code": result.returncode,
            "stdout": result.stdout[:1000] + ("..." if len(result.stdout) > 1000 else ""),
...
def create_node_red_exec_node_config(test_results):
    ...
    exec_node_config = {
        ...
        "timeout": str(int(test_results['execution_time_seconds'] * 3 + 60)),  # 3x test time + buffer
        "killSignal": "SIGTERM",
        "x": 400,
        "y": 200,
        "wires": [
