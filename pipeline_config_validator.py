...
class DataSourceConfig(BaseModel):
    ...
    def validate_enabled_sources(cls, v):
        ...
        supported_sources = [
            "racing_post", "betfair", "timeform", "itsracing", 
            "punters_intelligence", "geegeez"
        ]
        for source in v:
...
class ProcessingConfig(BaseModel):
    ...
    validation_threshold: float = Field(
        ...
        ge=0.5,
        le=1.0,
        description="Data validation threshold (0.5-1.0)"
    )
    ...
    max_processing_time_minutes: int = Field(
        ...
        ge=10,
        le=240,
        description="Maximum processing time per stage"
    )
...
class MLModelConfig(BaseModel):
    ...
    max_training_time_minutes: int = Field(
        ...
        ge=30,
        le=480,
        description="Maximum training time per model"
    )
    ...
    def validate_model_types(cls, v):
        ...
        supported_models = [
            ...
            "xgboost", "lightgbm", "svm", "logistic_regression"
        ]
        for model in v:
...
def main():
    ...
    try:
        ...
        invalid_config = {
            "schedule": {"download_time": "25:00"}  # Invalid time
        }
