...
def demonstrate_phase_2b_features():
    ...
    print(
        f"   🎯 Alert thresholds: CPU>{monitor.alert_manager.thresholds['cpu_critical']}%"
    )
    ...
    print(
        f"   Memory Usage:  {snapshot.memory_percent:5.1f}% ({snapshot.memory_mb:,.0f} MB)"
    )
    ...
    for stage_name in ["data_download", "data_validation", "data_cleaning"]:
        ...
        if "error" not in analysis:
            ...
            print(
                f"      Avg Processing Time: {analysis['processing_time']['mean']:.2f}s"
            )
            print(
                f"      Time Range: {analysis['processing_time']['min']:.2f}s - {analysis['processing_time']['max']:.2f}s"
            )
            print(
                f"      Avg Throughput: {analysis['throughput']['mean']:.1f} records/sec"
            )
    ...
    for stage, baseline in monitor.stage_baselines.items():
        print(
            f"      {stage:20} | {baseline.processing_time:6.2f}s | {baseline.throughput:6.1f} rec/s"
        )
    ...
    print(
        f"   Alert Types in Cooldown:     {len(monitor.alert_manager.alert_cooldown)}"
    )
    ...
    print(
        f"   • {len(monitor.alert_manager.alert_history)} intelligent alerts generated"
    )
