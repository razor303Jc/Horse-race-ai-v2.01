...
class SimpleAPIPostgreSQL:
    ...
    def initialize_pool(self):
        ...
        try:
            ...
            # Initialize pool
            self.pool = SimpleConnectionPool(minconn=2, maxconn=10, **POSTGRES_CONFIG)
    ...
    def _setup_routes(self):
        ...
        def database_status():
            ...
            try:
                # Get entity counts
                horses = self.execute_query(
                    "SELECT COUNT(*) as count FROM horses_entity"
                )
                jockeys = self.execute_query(
                    "SELECT COUNT(*) as count FROM jockeys_entity"
                )
                trainers = self.execute_query(
                    "SELECT COUNT(*) as count FROM trainers_entity"
                )

                total_records = (
                    horses[0]["count"] + jockeys[0]["count"] + trainers[0]["count"]
                )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "horses" in data:
                    ...
                    print(
                        f"    📊 Total records: {counts['total']} (H:{counts['horses']}, J:{counts['jockeys']}, T:{counts['trainers']})"
                    )
