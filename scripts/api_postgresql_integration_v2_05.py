...
class APIPostgreSQLIntegration:
    ...
    def _setup_routes(self):
        ...
        async def database_status():
            ...
            try:
                ...
                tables = ["horses_entity", "jockeys_entity", "trainers_entity", "races_entity"]
                for table in tables:
        ...
        async def get_horses(
            ...
            try:
                ...
                horses_params = tuple(params + [limit, offset]) if params else (limit, offset)
        ...
        async def get_jockeys(
            ...
            try:
                ...
                if min_win_rate:
                    where_conditions.append(f"(wins::float / NULLIF(runs, 0) * 100) >= ${param_count}")
                ...
                jockeys_params = tuple(params + [limit, offset]) if params else (limit, offset)
        ...
        async def get_trainers(
            ...
            try:
                ...
                if min_win_rate:
                    where_conditions.append(f"(wins::float / NULLIF(total_races, 0) * 100) >= ${param_count}")
                    params.append(min_win_rate)
                    param_count += 1
                ...
                trainers_params = tuple(params + [limit, offset]) if params else (limit, offset)
        ...
        async def get_top_performers(
            ...
            metric: str = Query("win_rate", regex="^(win_rate|total_wins|total_races)$"),
        ...
        async def get_analytics_statistics():
            ...
            try:
                ...
                response_time = (time.time() - start_time) * 1000
                self._track_endpoint_performance("get_analytics_statistics", response_time)
                
                return {
                    "statistics": detailed_stats,
                    "system_performance": {
                        "database_avg_query_time_ms": round(self.performance_metrics["database_stats"]["avg_query_time"], 2),
                        "total_database_queries": self.performance_metrics["database_stats"]["total_queries"],
                        "connection_pool_hits": self.performance_metrics["database_stats"]["connection_pool_hits"]
                    },
                    "timestamp": datetime.now().isoformat(),
                    "performance": {
                        "response_time_ms": round(response_time, 2)
        ...
        async def get_performance_metrics():
            ...
            return {
                ...
                "connection_pool": {
                    "min_connections": 2,
                    "max_connections": 20,
                    "current_hits": self.performance_metrics["database_stats"]["connection_pool_hits"]
                },
                "performance_targets": {
                    "avg_query_time_target": "< 5ms",
                    "api_response_target": "< 100ms",
                    "current_avg_query_time": round(self.performance_metrics["database_stats"]["avg_query_time"], 2),
                    "current_api_avg_response": round(self.performance_metrics["avg_response_time"], 2)
                },
                "timestamp": datetime.now().isoformat()
            }
    ...
    def _track_endpoint_performance(self, endpoint: str, response_time: float):
        ...
        endpoint_new_avg = ((endpoint_avg * (endpoint_total - 1)) + response_time) / endpoint_total
...
async def run_api_integration_tests():
    ...
    try:
        ...
        avg_response_time = sum([r.get("response_time_ms", 0) for r in test_results if "response_time_ms" in r]) / len([r for r in test_results if "response_time_ms" in r])
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                api_avg_time = metrics["performance_targets"]["current_api_avg_response"]
                
                print(f"  Database Avg Query Time: {db_avg_time}ms")
                print(f"  API Avg Response Time: {api_avg_time}ms")
                print(f"  Connection Pool Hits: {metrics['connection_pool']['current_hits']}")
