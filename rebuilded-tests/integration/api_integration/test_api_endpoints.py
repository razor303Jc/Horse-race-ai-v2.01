...
class TestPredictionAPIIntegration:
    ...
    @pytest.mark.integration
    @pytest.mark.api
    def test_race_prediction_endpoint(
        self, api_test_environment, sample_race_data, mock_api_responses
    ):
...
class TestMLManagementAPIIntegration:
    ...
    @pytest.mark.api
    @pytest.mark.ml
    def test_ml_training_trigger_endpoint(
        self, api_test_environment, mock_api_responses
    ):
    ...
    @pytest.mark.api
    @pytest.mark.ml
    def test_ml_training_status_endpoint(
        self, api_test_environment, mock_api_responses
    ):
...
class TestWebApplicationIntegration:
    ...
    def test_web_app_health_check(self, api_test_environment):
        ...
        with patch("requests.get") as mock_get:
            ...
            mock_response.status_code = 200
            mock_response.text = (
                "<html><head><title>Horse Racing AI</title></head></html>"
            )
            mock_get.return_value = mock_response
    ...
    def test_race_upload_interface(self, api_test_environment, sample_race_data):
        ...
        files = {
            "race_file": ("test_race.csv", "horse,jockey,odds\nHorse A,Jockey A,3.50")
        }
        ...
        with patch("requests.post") as mock_post:
            ...
            response = requests.post(
                url, files=files, timeout=api_test_environment["test_timeout"]
            )
...
class TestAPIIntegrationWithDatabase:
    ...
    def test_api_data_retrieval(self, api_test_environment, test_db_connection):
        ...
        query_params = {"race_date": "2025-01-02", "track": "RANDWICK", "limit": 10}
...
class TestAPIErrorHandlingAndSecurity:
    ...
    def test_api_input_validation(self, api_test_environment):
        ...
        malicious_data = {"race_id": "'; DROP TABLE races; --", "runners": []}
