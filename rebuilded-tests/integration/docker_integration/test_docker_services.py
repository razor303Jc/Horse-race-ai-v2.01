...
def docker_test_environment():
    ...
    return {
        ...
        "volumes": {"test_data": "/test_data", "test_models": "/test_models"},
        "timeout": 60,
    }
...
class TestDockerContainerIntegration:
    ...
    def test_container_health_checks(
        self, mock_docker_operations, docker_test_environment
    ):
        ...
        for service_name, config in docker_test_environment["test_containers"].items():
            ...
            health_status = (
                container_info.attrs["State"].get("Health", {}).get("Status")
            )
            ...
            assert health_status in [
                "healthy",
                None,
            ], f"Container {service_name} is unhealthy"
    ...
    def test_container_communication(
        ...
        network = client.networks.create(
            docker_test_environment["network_name"], driver="bridge"
        )
        ...
        db_container = client.containers.run(
            ...
            detach=True,
            environment=docker_test_environment["test_containers"]["postgres"][
                "environment"
            ],
            name="test_postgres",
        ...
        api_container = client.containers.run(
            ...
            detach=True,
            environment=docker_test_environment["test_containers"]["api"][
                "environment"
            ],
            name="test_api",
            network=docker_test_environment["network_name"],
    ...
    @pytest.mark.integration
    @pytest.mark.docker
    def test_container_volume_mounting(
        self, mock_docker_operations, docker_test_environment
    ):
    ...
    def test_docker_compose_service_health(self, docker_test_environment):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            mock_run.return_value.stdout = json.dumps(
                [
                    {"Name": "test_api_1", "State": "running", "Health": "healthy"},
                    {
...
class TestDockerNetworkingIntegration:
    ...
    @pytest.mark.docker
    @pytest.mark.network
    def test_docker_network_creation(
        self, mock_docker_operations, docker_test_environment
    ):
...
class TestDockerMonitoringIntegration:
    ...
    def test_container_metrics_collection(self, mock_docker_operations):
        ...
        # Start container
        container = client.containers.run(
            "alpine:latest", command="sleep 300", detach=True, name="test_metrics"
        )
