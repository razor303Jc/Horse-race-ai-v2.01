...
def large_dataset_generator():
    ...
    def generate_dataset(size: int = 10000) -> pd.DataFrame:
        ...
        data = {
            ...
            'track': np.random.choice(['RANDWICK', 'FLEMINGTON', 'ROSEHILL', 'CAULFIELD'], size),
            'distance': np.random.choice([1000, 1200, 1400, 1600, 2000], size),
            'track_condition': np.random.choice(['GOOD', 'SOFT', 'HEAVY', 'FIRM'], size),
...
class TestDataProcessingPerformance:
    ...
    def test_bulk_upload_performance(self, large_dataset_generator, performance_test_config, performance_monitor):
        ...
        for size in dataset_sizes:
            ...
            threshold = performance_test_config['benchmarks']['data_processing_threshold']
            assert records_per_second >= threshold, \
    ...
    @pytest.mark.performance
    @pytest.mark.benchmark
    def test_data_cleaning_performance(self, large_dataset_generator, performance_test_config):
        ...
        assert records_per_second >= 500, f"Data cleaning too slow: {records_per_second:.1f} records/sec"
    ...
    @pytest.mark.performance
    @pytest.mark.benchmark
    def test_feature_engineering_performance(self, large_dataset_generator, performance_test_config):
        ...
        assert records_per_second >= 200, f"Feature engineering too slow: {records_per_second:.1f} records/sec"
...
class TestMLPerformance:
    ...
    @pytest.mark.ml
    @pytest.mark.benchmark
    def test_model_training_performance(self, large_dataset_generator, performance_test_config, performance_monitor):
        ...
        for size in training_sizes:
            ...
            # Performance assertions
            assert training_time <= 120, f"Training too slow: {training_time:.1f}s for {size} records"
            assert training_results['accuracy'] >= 0.6
            
            # System resource checks
            system_stats = performance_monitor.get_stats()
            if system_stats:
                memory_threshold = performance_test_config['benchmarks']['memory_threshold']
                assert system_stats.get('max_memory_mb', 0) <= memory_threshold, \
    ...
    @pytest.mark.ml
    @pytest.mark.benchmark
    def test_prediction_performance(self, large_dataset_generator, performance_test_config):
        ...
        for batch_size in batch_sizes:
            ...
            assert predictions_per_second >= threshold, \
    ...
    @pytest.mark.ml
    @pytest.mark.benchmark
    def test_concurrent_prediction_performance(self, large_dataset_generator, performance_test_config):
        ...
        # Performance assertions
        assert overall_throughput >= 50, f"Concurrent throughput too low: {overall_throughput:.1f} pred/sec"
        assert avg_prediction_time <= 1.0, f"Average prediction time too high: {avg_prediction_time:.2f}s"
        assert max_prediction_time <= 5.0, f"Max prediction time too high: {max_prediction_time:.2f}s"
...
class TestAPIPerformance:
    ...
    def test_api_response_time_performance(self, performance_test_config):
        ...
        for endpoint in endpoints:
            ...
            for _ in range(10):
                with patch('requests.get' if endpoint == '/health' else 'requests.post') as mock_request:
                    mock_response = Mock()
    ...
    def test_api_load_performance(self, performance_test_config, performance_monitor):
        ...
        # Performance assertions
        assert requests_per_second >= 20, f"Request throughput too low: {requests_per_second:.1f} req/sec"
        ...
        if system_stats:
            ...
            assert system_stats.get('avg_cpu_percent', 0) <= cpu_threshold, \
...
class TestStressAndScalability:
    ...
    @pytest.mark.performance
    @pytest.mark.stress
    def test_memory_stress(self, large_dataset_generator, performance_test_config, performance_monitor):
        ...
        system_stats = performance_monitor.get_stats()
        if system_stats:
            memory_threshold = performance_test_config['benchmarks']['memory_threshold'] * 2  # Allow higher for stress test
            assert system_stats.get('max_memory_mb', 0) <= memory_threshold, \
                f"Memory usage exceeded stress threshold: {system_stats['max_memory_mb']:.1f}MB"
    
    @pytest.mark.performance
    @pytest.mark.stress
    def test_concurrent_processing_stress(self, large_dataset_generator, performance_test_config):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_concurrent) as executor:
            futures = [executor.submit(processing_worker) for _ in range(max_concurrent)]
        ...
        assert throughput >= 5, f"Concurrent processing throughput too low: {throughput:.1f} jobs/sec"
    ...
    def _simulate_feature_engineering(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in numeric_columns:
            if col in ['distance', 'weight', 'odds']:
                features_df[f'{col}_normalized'] = (data[col] - data[col].mean()) / data[col].std()
