...
class AdvancedMetricsPredictor:
    ...
    def predict_place_probability(self, race_data: pd.DataFrame) -> np.ndarray:
        ...
        probabilities = model.predict_proba(X)[
            :, 1
        ]  # Get probability of class 1 (place)
        return probabilities
    ...
    def make_comprehensive_predictions(self, race_data: pd.DataFrame) -> pd.DataFrame:
        ...
        if "win_probability" in prepared_data.columns:
            ...
            results["win_prob_diff"] = (
                results["ml_win_probability"] - results["original_win_prob"]
            )
    ...
    def analyze_race_predictions(self, results: pd.DataFrame) -> Dict:
        ...
        analysis = {
            ...
            "favorite_by_ml": results.loc[results["ml_win_probability"].idxmax()][
                "horse_name"
            ],
            "favorite_ml_prob": results["ml_win_probability"].max(),
            "predicted_winner_position": results.loc[
                results["ml_predicted_position"].idxmin()
            ]["horse_name"],
            "best_predicted_position": results["ml_predicted_position"].min(),
            "top_3_by_ml_win": results.nlargest(3, "ml_win_probability")[
                ["horse_name", "ml_win_probability"]
            ].to_dict("records"),
            "top_3_by_ml_place": results.nlargest(3, "ml_place_probability")[
                ["horse_name", "ml_place_probability"]
            ].to_dict("records"),
        }

        if "win_prob_diff" in results.columns:
            analysis["biggest_upgrade"] = results.loc[
                results["win_prob_diff"].idxmax()
            ]["horse_name"]
            analysis["biggest_upgrade_diff"] = results["win_prob_diff"].max()
            analysis["biggest_downgrade"] = results.loc[
                results["win_prob_diff"].idxmin()
            ]["horse_name"]
...
def test_with_sample_race():
    ...
    for record in data["current_data"][
        :12
    ]:  # First 12 horses (approximately 1-2 races)
    ...
    print(
        f"ML Favorite: {analysis['favorite_by_ml']} ({analysis['favorite_ml_prob']:.3f})"
    )
    print(
        f"Predicted Winner: {analysis['predicted_winner_position']} (Position {analysis['best_predicted_position']:.1f})"
    )
    ...
    if "biggest_upgrade" in analysis:
        ...
        print(
            f"📉 Biggest Downgrade: {analysis['biggest_downgrade']} ({analysis['biggest_downgrade_diff']:.3f})"
        )
    ...
    display_columns = [
        ...
        "power_rating",
        "ml_win_probability",
        "ml_predicted_position",
        "ml_place_probability",
    ]
    ...
    display_results = results[display_columns].sort_values(
        "ml_win_probability", ascending=False
    )
