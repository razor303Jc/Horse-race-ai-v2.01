    ...
    def load_production_models(self) -> bool:
        ...
        try:
            ...
            if self.metadata_file.exists():
                ...
                logger.info(
                    f"✅ Loaded metadata for {self.model_metadata.get('total_models', 0)} models"
                )
            ...
            if manifest_file.exists():
                ...
                for model_id in manifest.get("active_models", [])[
                    :3
                ]:  # Load first 3 models
                ...
                logger.info(
                    f"📊 Successfully loaded {len(self.loaded_models)} production models"
                )
    ...
    def create_prediction_api(self) -> FastAPI:
        ...
        async def list_models():
            ...
            for model_id, model_data in self.loaded_models.items():
                models_info[model_id] = {
                    ...
                    "feature_count": model_data["metadata"].get(
        ...
        async def predict_horse(features: HorseFeatures):
            ...
            try:
                ...
                feature_values = [
                    features.horse_age,
                    features.jockey_rating,
                    features.trainer_rating,
                    features.track_condition,
                    features.distance,
                    features.weight,
                    features.odds,
                    features.recent_form,
                    features.class_rating,
                    features.speed_rating,
                    features.stamina,
                    features.draw_position,
                    features.going_preference,
                    features.course_form,
                    features.distance_form,
                ]
                ...
                if len(feature_values) < expected_features:
                    feature_values.extend(
                        [0.0] * (expected_features - len(feature_values))
                    )
    ...
    def test_prediction_service(self) -> bool:
        ...
        try:
            ...
            if predict_response.status_code == 200:
                ...
                logger.info(
                    f"✅ Prediction endpoint working - Model: {result['model_used']}"
                )
