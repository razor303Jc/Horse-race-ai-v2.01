...
class Stage7TestFramework:
    ...
    def test_web_ui_functionality(self) -> bool:
        ...
        try:
            ...
            required_elements = ['<div id="root">', "Horse Racing", "Dashboard"]
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if any("web_container" in test["test_name"].lower() for test in failed_tests):
            recommendations.append(
                "Restart web container: docker restart horse_racing_web_app_clean"
            )

        if any("prediction_api" in test["test_name"].lower() for test in failed_tests):
            recommendations.append("Check prediction API service in ML container")

        if any("integration" in test["test_name"].lower() for test in failed_tests):
            recommendations.append("Verify network connectivity between containers")
