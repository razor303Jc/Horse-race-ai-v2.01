...
def find_latest_csv_files():
    ...
    racecard_files = list(downloads_dir.glob("cards_data/racecard_details/racecard_details.csv"))
    if racecard_files:
        csv_files["racecard_details"] = str(max(racecard_files, key=lambda f: f.stat().st_mtime))
