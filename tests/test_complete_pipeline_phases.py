    ...
    def setUpClass(cls):
        ...
        try:
            with open(cls.config_file, 'r', encoding='utf-8') as f:
                ...
                logger.info(f"✅ Loaded {len(cls.stages)} stages across {len(cls.phases)} phases")
...
class TestFeatureEngineeringPhase(CompletePipelinePhasesTest):
    ...
    def test_feature_engineering_stage(self):
        ...
        # Engineer features
        features = {
            "age_weight_ratio": race_data["horse"]["age"] / race_data["horse"]["weight"],
            "win_percentage": race_data["form"]["wins"] / race_data["form"]["runs"],
            "place_percentage": race_data["form"]["places"] / race_data["form"]["runs"],
            "rating_per_kg": race_data["horse"]["rating"] / race_data["horse"]["weight"],
            "distance_category": "sprint" if race_data["track"]["distance"] < 1400 else "middle"
        }
    ...
    def test_contextual_analysis_stage(self):
        ...
        insights = {
            "weather_impact": "Favorable" if context["weather"]["condition"] == "Fine" else "Adverse",
            "track_suitability": "Optimal" if context["track"]["condition"] == "Good" else "Suboptimal",
            "field_strength": "Strong" if context["field"]["competitiveness"] == "High" else "Weak",
...
class TestAdvancedAnalyticsPhase(CompletePipelinePhasesTest):
    ...
    def test_power_ratings_stage(self):
        ...
        for i, time in enumerate(horse_data["recent_times"]):
            adjusted_time = time + horse_data["weight_adjustments"][i] + horse_data["going_adjustments"][i]
    ...
    def test_speed_analysis_stage(self):
        ...
        average_sectional = sum(speed_data["sectional_times"]) / len(speed_data["sectional_times"])
        speed_per_meter = speed_data["final_time"] / speed_data["distance"]
        pace_variance = max(speed_data["sectional_times"]) - min(speed_data["sectional_times"])
...
class TestSimulationPhase(CompletePipelinePhasesTest):
    ...
    def test_monte_carlo_simulations_stage(self):
        ...
        total_win_probability = sum(horse["win_probability"] for horse in simulation_results.values())
        ...
        for horse_result in simulation_results.values():
            ...
            self.assertGreaterEqual(horse_result["place_probability"], horse_result["win_probability"])
    ...
    def test_race_trends_stage(self):
        ...
        trend_data = {
            ...
            "barrier_stats": {1: 0.18, 2: 0.15, 3: 0.13, 4: 0.12, 5: 0.10, 6: 0.09, 7: 0.08, 8: 0.07}
        }
        
        # Analyze trends
        dominant_track_position = max(trend_data["track_bias"], key=trend_data["track_bias"].get)
        preferred_runner_type = max(trend_data["distance_winners"], key=trend_data["distance_winners"].get)
        best_barrier = min(trend_data["barrier_stats"], key=trend_data["barrier_stats"].get)
        ...
        self.assertEqual(trends_analysis["barrier_advantage"], 8)  # Highest barrier number has lowest value
...
class TestStrategyPhase(CompletePipelinePhasesTest):
    ...
    def test_betting_strategies_stage(self):
        ...
        betting_data = {
            "Horse_1": {"probability": 0.35, "odds": 3.50, "composite_score": 0.85},  # Value bet
            "Horse_2": {"probability": 0.25, "odds": 5.00, "composite_score": 0.78},  # Value bet
    ...
    def test_ai_selections_stage(self):
        ...
        candidates = [
            {"horse": "Horse_1", "composite_score": 0.85, "confidence": 0.88, "value": 0.12},
            {"horse": "Horse_2", "composite_score": 0.78, "confidence": 0.82, "value": 0.08},
            {"horse": "Horse_3", "composite_score": 0.72, "confidence": 0.75, "value": 0.15},
            {"horse": "Horse_4", "composite_score": 0.65, "confidence": 0.68, "value": 0.05}
        ]
    ...
    def test_report_generation_stage(self):
        ...
        report_data = {
            ...
            "selections": [
                {"horse": "Thunder Strike", "jockey": "J. Smith", "barrier": 3, "rating": 0.85},
                {"horse": "Lightning Bolt", "jockey": "M. Jones", "barrier": 7, "rating": 0.78}
            ],
            "analysis": {
                "key_factors": ["Track bias favoring inside runners", "Consistent pace expected"],
        ...
        report_sections = {
            ...
            "top_selection": report_data["selections"][0]["horse"] if report_data["selections"] else None,
...
class TestPreRacePhase(CompletePipelinePhasesTest):
    ...
    def test_pre_race_updates_stage(self):
        ...
        live_updates = {
            "scratching": [],
            "jockey_changes": [{"horse": "Horse_3", "old_jockey": "J. Smith", "new_jockey": "M. Brown"}],
            "barrier_changes": [{"horse": "Horse_5", "old_barrier": 8, "new_barrier": 4}],
            "weight_changes": [],
...
class TestPipelineIntegration(CompletePipelinePhasesTest):
    ...
    def test_stage_dependencies(self):
        ...
        expected_order = [
            "data_download", "data_validation", "data_preprocessing", "data_relationships",
            "feature_engineering", "contextual_analysis", "form_scoring", 
            "power_ratings", "speed_analysis", "ml_model_training",
            "monte_carlo_simulations", "race_trends", "composite_scoring",
            "betting_strategies", "ai_selections", "report_generation", "pre_race_updates"
        ]
        ...
        data_stages = ["data_download", "data_validation", "data_preprocessing", "data_relationships"]
        ...
        for data_stage in data_stages:
            for feature_stage in feature_stages:
                if (data_stage in stage_indices and feature_stage in stage_indices and
                    stage_indices[data_stage] < stage_indices[feature_stage]):
    ...
    def test_pipeline_timing_constraints(self):
        ...
        critical_stages = [stage for stage in self.stages if stage.get("critical", False)]
        ...
        for stage in self.stages:
            phase = stage["phase"]
            phase_durations[phase] = phase_durations.get(phase, 0) + stage["duration_minutes"]
        
        # ML training should be the longest single phase component
        ml_duration = next(
            stage["duration_minutes"] for stage in self.stages 
    ...
    def test_error_recovery_mechanisms(self):
        ...
        critical_stages = [stage for stage in self.stages if stage.get("critical", True)]
        optional_stages = [stage for stage in self.stages if not stage.get("critical", True)]
