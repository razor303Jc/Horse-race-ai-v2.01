...
class TestCSVColumnMapping:
    ...
    def test_separated_card_results_architecture(self, sample_csv_mapping):
        ...
        for table in card_tables:
            if table in tables:
                ...
                assert (
                    "race_id" in tables[table]
                    or "record_id" in tables[table]
                    or "horse_id" in tables[table]
                )
        ...
        for table in result_tables:
            if table in tables:
                ...
                primary_keys = [
                    col for col, def_ in tables[table].items() if "PRIMARY KEY" in def_
                ]
    ...
    def test_data_type_mapping(self, sample_csv_mapping):
        ...
        for table_name, table_schema in tables.items():
            for column_name, column_def in table_schema.items():
                ...
                assert (
                    has_valid_type
                ), f"Invalid data type in {table_name}.{column_name}: {column_def}"
    ...
    def test_primary_key_constraints(self, sample_csv_mapping):
        ...
        for table_name, table_schema in tables.items():
            ...
            primary_keys = [
                col for col, def_ in table_schema.items() if "PRIMARY KEY" in def_
            ]
            assert (
                len(primary_keys) == 1
            ), f"Table {table_name} should have exactly one primary key"
        ...
        for table_name, csv_file in sample_csv_data.items():
            if table_name in sample_csv_mapping["tables"]:
                ...
                for pk_col in primary_key_cols:
                    assert (
                        pk_col in csv_columns
                    ), f"Primary key {pk_col} missing in {table_name} CSV"
        ...
        for table_name, table_schema in tables.items():
            for column_name, column_def in table_schema.items():
                for indicator in foreign_key_indicators:
                    assert (
                        indicator not in column_def.upper()
