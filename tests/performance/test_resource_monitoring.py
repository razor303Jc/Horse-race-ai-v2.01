...
class ResourceMonitor:
        ...
        try:
            ...
            return {
                ...
                'open_files': (self.process.num_fds() 
                              if hasattr(self.process, 'num_fds') else 0),
                'connections': len(self.process.connections()),
                'system_memory_percent': psutil.virtual_memory().percent,
                'system_cpu_percent': psutil.cpu_percent(),
                'disk_io_read_mb': (psutil.disk_io_counters().read_bytes 
                                   / 1024 / 1024 if psutil.disk_io_counters() else 0),
    ...
    def get_resource_statistics(self) -> Dict:
        ...
        stats = {}
        metrics = ['cpu_percent', 'memory_rss_mb', 'memory_vms_mb', 'threads', 
                  'open_files', 'connections', 'system_memory_percent', 
                  'system_cpu_percent']
        ...
        stats['monitoring_duration'] = (valid_history[-1]['timestamp'] - 
                                       valid_history[0]['timestamp'])
...
class TestResourceMonitoring:
    ...
    def test_orchestrator_initialization_resource_impact(self):
        ...
        print(f"   🧵 Thread increase: "
              f"{final_resources['threads'] - baseline['threads']}")
        print(f"   📁 Open files increase: "
        ...
        assert stats['cpu_percent']['max'] < 150, \
            f"CPU usage during initialization {stats['cpu_percent']['max']:.1f}% too high"
        assert final_resources['threads'] - baseline['threads'] < 10, \
    ...
    def test_database_connection_resource_usage(self):
        ...
        memory_variance = (stats['memory_rss_mb']['max'] - 
                          stats['memory_rss_mb']['min'])
    ...
    def test_memory_leak_detection(self):
        ...
        stats = self.resource_monitor.get_resource_statistics()
        memory_samples = [r['memory_rss_mb'] for r in self.resource_monitor.resource_history 
    ...
    def test_file_handle_management(self):
        ...
        assert file_increase < 10, \
            f"File handle leak detected: {file_increase} handles not cleaned up"
        assert stats['open_files']['max'] - baseline_files <= 25, \
