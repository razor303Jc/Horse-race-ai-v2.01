...
def run_performance_tests(test_type: str = "all", verbose: bool = True):
    ...
    for test_key in tests_to_run:
        ...
        if result == 0:
            ...
            print(
                f"❌ {test_info['name']} FAILED (exit code: {result}, {execution_time:.2f}s)"
            )
    ...
    results_file = (
        ENVIRONMENT_CONFIG["paths"]["reports_dir"] / "performance_test_results.json"
    )
    try:
        import json
...
Examples:
  ...
  python run_performance_tests.py --type benchmark --quiet
    ...
    if args.config_check:
        ...
        print(f"   👥 Workers: {config['execution']['parallel_workers']}")
        print(
            f"   🎯 Success threshold: {config['thresholds']['success_rate_min']:.1%}"
        )
