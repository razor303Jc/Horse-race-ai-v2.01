...
class LoadTestingFramework:
    ...
    def generate_large_dataset(
        self, num_races: int = 1000, num_horses_per_race: int = 12
    ) -> str:
            ...
            for horse_id in range(num_horses_per_race):
                writer.writerow(
                    [
                        ...
                        race_name,
                        race_class,
                        f"Horse_{race_id}_{horse_id}",
                        f"Jockey_{horse_id}",
                        f"Trainer_{horse_id}",
                        f"{55 + horse_id}kg",
                        f"{2.5 + horse_id * 0.5:.1f}",
                        horse_id + 1,
                        85 + horse_id * 2,
                    ]
                )
...
class TestLoadPerformance:
    ...
    def test_startup_performance_under_load(self, mock_pool, mock_config_manager):
        ...
        assert (
            avg_startup_time < 0.1
        ), f"Average startup time {avg_startup_time:.3f}s exceeds 100ms threshold"
        assert (
            max_startup_time < 0.2
        ), f"Maximum startup time {max_startup_time:.3f}s exceeds 200ms threshold"
        assert (
            avg_memory < 100
        ), f"Average memory usage {avg_memory:.1f}MB exceeds 100MB threshold"
    ...
    def test_large_dataset_processing_performance(self, mock_pool, mock_config_manager):
        ...
        for size in test_sizes:
            ...
            large_dataset = self.load_framework.generate_large_dataset(
                num_races=size, num_horses_per_race=12
            )
            ...
            # Calculate metrics
            processing_rate = (
                processed_rows / processing_time if processing_time > 0 else 0
            )
            memory_increase = (
            ...
            ), f"Processing time {processing_time:.3f}s too slow for {size} races"
            assert (
                processing_rate > 1000
            ), f"Processing rate {processing_rate:.1f} rows/s below 1000 rows/s threshold"
            assert (
                memory_increase < 50
    ...
    def test_database_connection_pool_under_load(self, mock_pool, mock_config_manager):
        ...
        if connection_times:
            ...
            # Performance assertions
            assert (
                successful_connections >= num_connections * 0.95
            ), "Less than 95% connection success rate"
            assert (
                avg_connection_time < 0.05
            ), f"Average connection time {avg_connection_time:.4f}s exceeds 50ms"
            assert (
                max_connection_time < 0.1
            ), f"Maximum connection time {max_connection_time:.4f}s exceeds 100ms"
        else:
    ...
    def test_memory_usage_stability_under_load(self, mock_pool, mock_config_manager):
        ...
        for i in range(num_iterations):
            ...
            try:
                ...
                # Monitor memory
                current_memory = self.load_framework.monitor_system_resources()[
                    "memory_mb"
                ]
        ...
        assert (
            memory_increase < 100
        ), f"Memory increase {memory_increase:.1f}MB exceeds 100MB threshold"
        assert (
            memory_variance < 50
        ), f"Memory variance {memory_variance:.1f}MB indicates instability"
        assert (
            final_memory < baseline_memory + 30
        ), f"Final memory {final_memory:.1f}MB shows potential memory leak"
