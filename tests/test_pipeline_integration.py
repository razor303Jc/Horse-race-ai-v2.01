...
class TestEnhancedFileWatcher:
    ...
    @patch(
        "tools.automation.file_watcher_enhanced.EnhancedFileWatcher._run_database_upload"
    )
    @patch(
        "tools.automation.file_watcher_enhanced.EnhancedFileWatcher._trigger_ml_updates"
    )
...
class TestPipelineAutomation:
    ...
    @patch(
        "tools.automation.pipeline_automation.PipelineAutomation._run_database_upload"
    )
    @patch(
        "tools.automation.pipeline_automation.PipelineAutomation._trigger_ml_updates"
    )
...
class TestDatabaseUploader:
    ...
    def test_database_connection(self, mock_connect):
        ...
        from tools.data_processing.simple_database_uploader import (
            SimpleDatabaseUploader,
        )
    ...
    def test_manifest_format_handling(self, temp_directory):
        ...
        manifest_old = {
            "files": [{"file": "result_races.csv", "table": "result_races"}]
        }

        # Create test manifest with "tables" format
        manifest_new = {"tables": {"result_races": {"file": "result_races.csv"}}}
...
class TestIntegrationScenarios:
    ...
    def test_error_recovery(self, temp_directory):
        ...
        assert (
            len(zip_files) == 1
        )  # File is detected but processing would fail gracefully
