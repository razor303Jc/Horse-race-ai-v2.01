...
class TestMainFunction:
    ...
    def test_main_success_with_updates(self, mock_update_trainers, mock_update_jockeys,
                                      mock_update_horses, mock_get_stats, mock_get_conn):
    ...
    @patch('scripts.update_index_tables.update_jockeys_index')
    @patch('scripts.update_index_tables.update_trainers_index')
    def test_main_no_updates_needed(self, mock_update_trainers, mock_update_jockeys,
                                   mock_update_horses, mock_get_stats, mock_get_conn):
    ...
    def test_main_database_error(self, mock_get_conn):
        ...
        try:
            ...
            pytest.fail(f"main() should handle database errors gracefully, but raised: {e}")
...
def sample_race_results_data():
    ...
    return [
        (1, 1, 1, 1, 101, 'New Horse A', 'GB', 4, 135, 5.0, 'New Jockey A', 'New Trainer A', 'F', 0),
        (2, 2, 2, 2, 102, 'New Horse B', 'IRE', 5, 140, 10.0, 'New Jockey B', 'New Trainer B', '2', 1.5),
        (3, 3, 999, 3, 103, 'New Horse C', 'FR', 3, 132, 15.0, 'New Jockey C', 'New Trainer C', '3', 999)
    ]
