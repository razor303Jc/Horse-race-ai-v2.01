...
class TestEntityLoaderFunctions:
    ...
    def test_load_horses_data_success(self, mock_exists, mock_read_csv, mock_execute_sql, mock_csv_data):
    ...
    @patch('pandas.read_csv')
    @patch('pathlib.Path.exists')
    def test_load_results_data_with_place_codes(self, mock_exists, mock_read_csv, mock_execute_sql, mock_csv_data):
...
class TestIndexTableUpdates:
    ...
    @patch('psycopg2.connect')
    def test_update_index_tables_no_new_entities(self, mock_connect, mock_db_connection):
...
class TestEntityLoaderIntegration:
    ...
    def test_main_pipeline_success(self, mock_verify, mock_update_index, mock_load_results,
                                  mock_load_races, mock_load_trainers, mock_load_jockeys, 
                                  mock_load_horses):
