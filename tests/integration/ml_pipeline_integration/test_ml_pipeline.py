...
def mock_ml_pipeline():
    ...
    with patch('src.ml_training.ml_pipeline.MLPipeline') as mock_pipeline:
        ...
        mock_instance.feature_engineer.return_value.columns = [
            'distance_normalized', 'weight_normalized', 'odds_log',
            'barrier_position_factor', 'jockey_performance_score',
        ...
        mock_instance.train_model.return_value = {
            ...
            'performance_metrics': {'accuracy': 0.72, 'precision': 0.68, 'recall': 0.71}
        }
...
class TestMLPipelineIntegration:
    ...
    def test_feature_engineering_integration(self, ml_training_data):
        ...
        expected_features = [
            ...
            'barrier_position_factor', 'jockey_performance_score',
            'trainer_performance_score', 'track_suitability_score',
            'form_momentum_score', 'class_rating_adjustment'
        ]
        ...
        assert not engineered_features.isnull().any().any(), "Features contain null values"
        assert np.isfinite(engineered_features.select_dtypes(include=[np.number]).values).all(), \
    ...
    @pytest.mark.integration
    @pytest.mark.ml
    def test_prediction_pipeline_integration(self, ml_training_data, ml_test_environment):
        ...
        batch_predictions = self._simulate_batch_predictions(model_results['model'], batch_data)
    ...
    def test_race_prediction_integration(self, ml_training_data, ml_test_environment):
        ...
        race_predictions = self._simulate_race_prediction(model_results['model'], race_data)
        ...
        assert np.isclose(np.sum(probabilities), 1.0, rtol=0.01)  # Probabilities sum to 1
    ...
    def test_model_persistence_integration(self, ml_training_data, ml_test_environment):
        ...
        metadata_path = ml_test_environment['models_dir'] / 'persistence_test_metadata.json'
    ...
    def _simulate_feature_engineering(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        features_df['weight_normalized'] = (data['weight'] - data['weight'].mean()) / data['weight'].std()
    ...
    def _simulate_model_training(self, features_df: pd.DataFrame, algorithm: str = 'RandomForest') -> Dict:
    ...
    def _simulate_batch_predictions(self, model: Mock, data: pd.DataFrame) -> List[float]:
    ...
    def _simulate_race_prediction(self, model: Mock, race_data: pd.DataFrame) -> Dict:
        ...
        recommended_bets = []
        for i, (pred, prob, conf) in enumerate(zip(raw_predictions, probabilities, confidence_scores)):
    ...
    def _simulate_cross_validation(self, features_df: pd.DataFrame, k_folds: int = 5) -> Dict:
        ...
        return {
            ...
            'fold_predictions': [np.random.uniform(0.1, 0.9, 50) for _ in range(k_folds)]
        }
...
class TestMLIntegrationWithDatabase:
    ...
    @pytest.mark.ml
    @pytest.mark.database
    def test_ml_predictions_storage_integration(self, test_db_connection, ml_test_environment):
        ...
        retrieved_predictions = self._simulate_predictions_retrieval(test_db_connection, 'RACE_001')
    ...
    def _simulate_predictions_storage(self, db_connection, predictions_df: pd.DataFrame) -> bool:
        """Simulate storing predictions in database"""
        # This would normally execute INSERT statements
        return True
    
    def _simulate_predictions_retrieval(self, db_connection, race_id: str) -> pd.DataFrame:
