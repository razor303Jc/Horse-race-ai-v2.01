...
class TestSimpleDatabaseUploader:
    ...
    def test_manifest_loading_old_format(
        self, temp_directory, sample_manifest_old_format
    ):
    ...
    def test_manifest_loading_new_format(
        self, temp_directory, sample_manifest_new_format
    ):
    ...
    def test_manifest_format_flexibility(self, temp_directory):
        ...
        # Test old format
        old_manifest = {"files": [{"file": "test.csv", "table": "test_table"}]}
        old_file = Path(temp_directory) / "old_manifest.json"
        with open(old_file, "w") as f:
            json.dump(old_manifest, f)

        # Test new format
        new_manifest = {"tables": {"test_table": {"file": "test.csv"}}}
...
class TestUploadManifestHandling:
    ...
    def test_manifest_validation(
        self, sample_manifest_old_format, sample_manifest_new_format
    ):
    ...
    def test_manifest_file_paths(self, temp_directory):
        ...
        manifest = {"tables": {"test_table": {"file": "data/test.csv"}}}
...
class TestDatabaseSchemaHandling:
    ...
    def test_no_foreign_key_constraints(self):
        ...
        sample_schemas = {
            "races": {"race_id": "INTEGER PRIMARY KEY", "race_name": "TEXT NOT NULL"},
            "horses": {
        ...
        if config_file.exists():
            ...
            if "tables" in config_data:
                ...
                separated_indicators = any(
                    any(
                        indicator in name
                        for indicator in ["card_", "result_", "jockeys_", "trainers_"]
                    )
