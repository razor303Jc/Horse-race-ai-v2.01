...
class TestEarlyMorningMLTrainer(unittest.TestCase):
    ...
    async def test_preconditions_validation(self):
        ...
        with patch.object(trainer, '_verify_data_download_completion', new_callable=AsyncMock):
            try:
...
class TestPipelineIntegration(unittest.TestCase):
    ...
    async def test_pipeline_timing_calculation(self):
        ...
        with patch('docker.ml_training.pipeline_integration.PipelineTimeAllocator') as mock_allocator:
            ...
            mock_allocator.return_value.allocate_stage_times.return_value = mock_schedule
...
class TestIntegrationScenarios(unittest.TestCase):
    ...
    @patch('docker.ml_training.pipeline_integration.PipelineTimeAllocator')
    @patch('docker.ml_training.early_morning_ml_trainer.PipelineTimeAllocator')
    async def test_complete_early_morning_workflow(self, mock_trainer_allocator, mock_integration_allocator):
        ...
        mock_integration_allocator.return_value.allocate_stage_times.return_value = mock_schedule
        ...
        with patch.dict(os.environ, {
            ...
            with patch.object(integration, '_execute_ml_training', new_callable=AsyncMock) as mock_training:
...
class TestDockerCompatibility(unittest.TestCase):
    ...
    def test_docker_path_handling(self):
        ...
        self.assertTrue(config.data_path.startswith('/app/') or not config.data_path.startswith('/'))
        self.assertTrue(config.models_path.startswith('/app/') or not config.models_path.startswith('/'))
        self.assertTrue(config.logs_path.startswith('/app/') or not config.logs_path.startswith('/'))
