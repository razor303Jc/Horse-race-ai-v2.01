module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    shell: {
      ...
      checkDatabase: {
        command:
          'cd ../.. && python -c "from src.web.api_server_enhanced import get_db_connection; print(\\"✅ Database connection OK\\" if get_db_connection() else \\"❌ Database connection failed\\")"',
        options: {
          stdout: true,
          stderr: true,
        },
      ...
      clean: {
        command:
          'rm -rf dist/ node_modules/.vite/ && echo "🧹 Cleaned build artifacts"',
        options: {
          stdout: true,
        },
      },
      ...
      openBrowser: {
        command:
          "open http://localhost:5003 || xdg-open http://localhost:5003 || start http://localhost:5003",
        options: {
          stdout: true,
        },
      },
  ...
  grunt.registerTask(
    "auto",
    "Start auto-reload development environment",
    function () {
