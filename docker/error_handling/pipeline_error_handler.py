...
class PipelineErrorHandler:
    ...
    def classify_error(
        ...
        if any(
            ...
            for keyword in ["connection", "timeout", "network", "http", "ssl", "socket"]
        ...
        if any(
            keyword in error_message
            for keyword in ["database", "sql", "postgres", "cursor", "transaction"]
        ...
        if any(
            keyword in error_message
            for keyword in ["file", "directory", "permission", "disk", "no such file"]
        ...
        if (
            ...
            or error_type in ["ValueError", "RuntimeError"]
            and context
            and "ml" in str(context)
        ):
        ...
        if any(
            keyword in error_message
            for keyword in ["validation", "invalid", "missing", "null", "empty"]
        ...
        if any(
            keyword in error_message
            for keyword in ["api", "request", "response", "status", "404", "500"]
        ):
        ...
        if any(
            keyword in error_message
            for keyword in ["memory", "cpu", "resource", "system", "process"]
        ...
        try:
            ...
            response = requests.post(
                ...
                headers={
                    ...
                    "Priority": (
                        "5" if error_context.severity == ErrorSeverity.CRITICAL else "4"
                    ),
    ...
    def retry_with_backoff(
        self, func: Callable, *args, context_data: Dict[str, Any] = None, **kwargs
    ) -> Any:
        """
        ...
        for attempt in range(
            max(config.max_retries for config in self.retry_configs.values()) + 1
        ):
            try:
                ...
                if error_context:
                    ...
                    logger.info(
                        f"✅ Error {error_context.error_id} resolved after {attempt} retries"
                    )
                ...
                if (
                    not self.should_retry(error_context)
                    or attempt >= error_context.max_retries
                ):
                    # Log final failure
                    self.log_error(error_context)
                    self.send_alert(error_context)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"📊 Error classified as: {context.category.value} / {context.severity.value}"
        )
