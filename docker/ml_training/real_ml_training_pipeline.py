...
class RealMLTrainingPipeline:
    ...
    def check_database_connection(self):
        ...
        try:
            ...
            self.logger.info(f"📊 Database status: {race_count} races, {record_count} records, {horse_count} horses")
    ...
    def train_models(self, training_data):
        ...
        try:
            ...
            rf_path = self.models_path / f'random_forest_win_predictor_{datetime.now().strftime("%Y%m%d_%H%M%S")}.joblib'
            joblib.dump(rf_model, rf_path)
            self.logger.info(f"✅ Random Forest: Accuracy={rf_accuracy:.3f}, AUC={rf_auc:.3f}, Saved to {rf_path}")
            models_trained += 1
            ...
            lr_path = self.models_path / f'logistic_regression_win_{datetime.now().strftime("%Y%m%d_%H%M%S")}.joblib'
            joblib.dump(lr_model, lr_path)
            self.logger.info(f"✅ Logistic Regression: Accuracy={lr_accuracy:.3f}, AUC={lr_auc:.3f}, Saved to {lr_path}")
            models_trained += 1
            ...
            self.logger.info(f"🎯 Ensemble: Accuracy={ensemble_accuracy:.3f}, AUC={ensemble_auc:.3f}")
    ...
    def run_real_training_cycle(self):
        ...
        try:
            ...
            if not self.check_database_connection():
                self.logger.warning("⚠️ No database connection or data available, skipping training")
                return False
    ...
    def run(self):
        ...
        try:
            while self.is_running:
                ...
                if training_success:
                    self.logger.info("🎯 Training successful, waiting 30 minutes before next cycle...")
                    wait_time = 1800  # 30 minutes
                else:
                    self.logger.info("⚠️ Training failed, waiting 10 minutes before retry...")
