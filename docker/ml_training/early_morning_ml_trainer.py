...
class TrainingMetrics:
    ...
    session_id: str = field(default_factory=lambda: f"early_ml_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
...
class DockerLogger:
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        file_handler = logging.FileHandler(
            log_dir / f"{self.component_name}_detailed.log",
            mode='a'
        )
        ...
        error_handler = logging.FileHandler(
            log_dir / f"{self.component_name}_errors.log",
            mode='a'
        )
...
class EarlyMorningMLTrainer:
    ...
    async def _verify_data_download_completion(self):
        ...
        if not any(marker.exists() for marker in download_markers):
            self.logger.warning("No download completion markers found, assuming data is ready")
            self.metrics.data_download_complete = True
            self.metrics.validation_passed = True
    
    async def _execute_training_cycles(self):
        """Execute the main training cycles"""
        self.logger.info(f"🚀 Starting {self.config.max_training_cycles} training cycles")
        
        for cycle in range(self.config.max_training_cycles):
            cycle_start = time.time()
            
            try:
                self.logger.info(f"🔄 Starting training cycle {cycle + 1}/{self.config.max_training_cycles}")
                ...
                self.logger.performance(
                    f"Training cycle {cycle + 1} completed",
        ...
        self.metrics.total_cycles = self.metrics.successful_cycles + self.metrics.failed_cycles
    ...
    async def _execute_single_cycle(self, cycle: int):
        ...
        try:
            ...
            if training_task in done:
                ...
                raise TimeoutError(f"Training cycle {cycle + 1} timed out after {self.config.timeout_seconds}s")
    ...
    async def _generate_training_report(self):
        ...
        # Save report
        reports_dir = Path("/app/reports") if Path("/app/reports").exists() else Path("reports")
        reports_dir.mkdir(exist_ok=True)
        
        report_file = reports_dir / f"early_morning_training_{self.metrics.session_id}.json"
...
async def main():
    ...
    config.max_training_cycles = int(os.environ.get('ML_MAX_CYCLES', config.max_training_cycles))
