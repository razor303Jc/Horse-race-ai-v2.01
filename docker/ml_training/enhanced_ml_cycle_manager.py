...
class DockerLoggingSetup:
    ...
    def setup_logging(component_name: str = "early_morning_ml") -> logging.Logger:
        ...
        file_handler = logging.FileHandler(
            log_dir / f"{component_name}_detailed.log", mode="a", encoding="utf-8"
        )
        ...
        error_handler = logging.FileHandler(
            log_dir / f"{component_name}_errors.log", mode="a", encoding="utf-8"
        )
        ...
        perf_handler = logging.FileHandler(
            log_dir / f"{component_name}_performance.log", mode="a", encoding="utf-8"
        )
    ...
    async def get_connection(self):
        ...
        while retry_count < self.max_retries:
            try:
                ...
                logger.debug(
                    f"Database connection established (attempt {retry_count + 1})"
                )
                yield connection
                return
...
class ErrorHandler:
    ...
    def categorize_error(self, error: Exception) -> str:
        ...
        for category, keywords in self.error_categories.items():
            if any(
                keyword in error_str or keyword in error_type for keyword in keywords
            ):
                return category
    ...
    def get_recovery_strategy(self, error: Exception) -> Dict[str, Any]:
        ...
        strategies = {
            "database": {"wait_time": 5.0, "max_retries": 5, "action": "reconnect"},
            "memory": {
                "wait_time": 10.0,
                "max_retries": 2,
        ...
        return strategies.get(
            category, {"wait_time": 3.0, "max_retries": 2, "action": "generic_retry"}
        )
...
class EnhancedMLCycleManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(
            f"Training window: {self.early_morning_start} - {self.early_morning_end}"
        )
...
async def main():
    ...
    try:
        ...
        if start_time <= current_time <= end_time:
            ...
            logger.info(
                f"⏰ Outside training window (current: {current_time}, window: {start_time}-{end_time})"
            )
